package luckStar.view
{
   ...
   public class LuckStarRankView extends Sprite implements Disposeable
   {
      ...
      private function init() : void
      {
         ...
         var addNum:int = 0;
         var i:int = 0;
         var rankItem:LuckStarRankItem = null;
         var awardItem:LuckStarNewAwardItem = null;
         ...
         this._awardNmae.text = LanguageMgr.GetTranslation("civil.leftview.playerName");
         ...
         for(i = 0; i < MAX_RANK_VIEW; i++)
         {
            ...
            itemY += addNum;
            addChild(rankItem);
            this._rankInfo.push(rankItem);
         }
         this._newAwardList = new Vector.<LuckStarNewAwardItem>();
         for(i = 0; i < MAX_AWARD; i++)
         {
            awardItem = new LuckStarNewAwardItem();
            this._textHeight = awardItem.height;
            awardItem.y = this._textHeight * i;
            this._newAward.addChild(awardItem);
            this._newAwardList.push(awardItem);
         }
      ...
      public function insertNewAwardItem(name:String, award:int, count:int) : void
      {
         ...
         awardItem.y = this._newAwardList[this._newAwardList.length - 1].y + this._textHeight;
         this._newAward.addChild(awardItem);
         this._newAwardList.push(awardItem);
      ...
      private function __onMove(e:Event) : void
      {
         ...
         var len:int = int(this._newAwardList.length);
         for(var i:int = 0; i < len; i++)
         {
            this._newAwardList[i].y -= 1;
         }
         this.check();
      }
      ...
      private function __onPreClick(e:MouseEvent) : void
      {
         ...
         var index:int = this._currentPage - 1;
         if(index < 1)
         {
            index = this._maxPage;
         }
      ...
      public function updateRankInfo() : void
      {
         ...
         var info:Vector.<LuckStarPlayerInfo> = LuckStarManager.Instance.model.rank[this._currentPage - 1] as Vector.<LuckStarPlayerInfo>;
         for(var i:int = 0; i < MAX_RANK_VIEW; i++)
         {
            this._rankInfo[i].resetItem();
            if(i < info.length && info[i] != null)
            {
               this._rankInfo[i].info = info[i];
            }
         }
      }
      ...
      public function updateSelfInfo() : void
      {
         ...
         if(Boolean(info))
         {
            this._myRankText.text = info.rank.toString();
            this._luckyStarNumText.text = info.starNum.toString();
         }
      }
      ...
      public function updateActivityDate() : void
      {
         ...
         if(Boolean(arr))
         {
            ...
            this._timeText.text = LanguageMgr.GetTranslation("ddt.luckStar.activityTime",str1,str2);
         }
      }
      ...
      public function dispose() : void
      {
         ...
         this._helpText = null;
         while(Boolean(this._rankInfo.length))
         {
            ObjectUtils.disposeObject(this._rankInfo.pop());
         }
         this._rankInfo = null;
         while(Boolean(this._newAwardList.length))
         {
