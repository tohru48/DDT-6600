package luckStar.view
{
   ...
   public class LuckStarFrame extends Frame
   {
      ...
      {
         var cell:LuckStarCell = null;
         AddPublicTipManager.Instance.type = AddPublicTipManager.XINGYUNXING;
         ...
         for(var i:int = 0; i < MAX_CELL; i++)
         {
            cell = new LuckStarCell();
            cell.selected = false;
            PositionUtils.setPos(cell,"luckyStar.view.cellPos" + i);
            cell.addEventListener(LuckStarEvent.LUCKYSTAR_EVENT,this.__onPlayActionEnd);
      ...
      {
         for(var i:int = 0; i < this._cell.length; i++)
         {
            if(this._cell[i].info.TemplateID == goods.TemplateID)
            {
               this._select = i;
            }
         }
         this._rewardList.push(goods);
         this.startTurn();
      }
      
      private function __onPlayActionEnd(e:LuckStarEvent) : void
      {
         if(e.code == LuckStarEvent.CELL_ACTION_COMPLETE)
         {
      ...
      private function showAwardFrame() : void
      {
         ...
         aView.goodsList = this._rewardList;
         aView.boxType = 4;
      ...
      {
         var _shopItemInfo:ShopItemInfo = ShopManager.Instance.getShopItemByTemplateID(EquipType.LUCKYSTAR_ID,3);
      ...
      {
         var count:int = PlayerManager.Instance.Self.PropBag.getItemCountByTemplateId(EquipType.LUCKYSTAR_ID);
         if(this._luckyStarCount != count)
         {
            this._luckyStarCount = count;
      ...
      {
         var len:int = int(LuckStarManager.Instance.model.goods.length);
         if(!this._cell)
         {
            return;
         }
         var i:int = 0;
         while(i < MAX_CELL && i < len)
         {
            this._cell[i].info = LuckStarManager.Instance.model.goods[i];
            this._cell[i].info.Quality = ItemManager.Instance.getTemplateById(this._cell[i].info.TemplateID).Quality;
            this._cell[i].count = LuckStarManager.Instance.model.goods[i].Count;
            i++;
         }
      }
      ...
      {
         this._rankView.insertNewAwardItem(name,award,count);
      }
      ...
      override public function dispose() : void
      {
         ...
         this._stopBtn = null;
         while(Boolean(this._cell.length))
         {
            cell = this._cell.pop();
            cell.removeEventListener(LuckStarEvent.LUCKYSTAR_EVENT,this.__onPlayActionEnd);
