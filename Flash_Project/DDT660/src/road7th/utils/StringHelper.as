package road7th.utils
{
   ...
   public class StringHelper
   {
      ...
      {
         var _loc4_:uint = Boolean(param1.text) ? uint(param1.text.match(reg).join("").length) : 0;
         var _loc5_:uint = Boolean(param3) ? uint(param3.match(reg).join("").length) : 0;
         param1.maxChars = param2 > _loc4_ + _loc5_ ? param2 - _loc4_ - _loc5_ : (param2 > _loc5_ ? param2 - _loc5_ : int(param2 / 2));
      }
      ...
      public static function reverseHtmlTextField(param1:String) : String
      {
         ...
         return param1.replace(_reverseRightReg,">");
      }
      ...
      {
         if(Boolean(idR1.test(param1)) || Boolean(idR2.test(param1)))
         {
            return true;
         }
         return false;
