package drgnBoat.views
{
   ...
   public class DrgnBoatThreeBtnView extends Sprite implements Disposeable
   {
      ...
      private function initView() : void
      {
         ...
         this._bg = ComponentFactory.Instance.creatBitmap("drgnBoat.threeBtnBg");
         this._leapBtn = ComponentFactory.Instance.creatComponentByStylename("drgnBoat.leapBtn");
         this._leapBtn.tipData = LanguageMgr.GetTranslation("drgnBoat.game.leapBtnTipTxt");
         this._invisibilityBtn = ComponentFactory.Instance.creatComponentByStylename("drgnBoat.invisibilityBtn");
         this._invisibilityBtn.tipData = LanguageMgr.GetTranslation("drgnBoat.game.invisibilityBtnTipTxt");
         this._cleanBtn = ComponentFactory.Instance.creatComponentByStylename("drgnBoat.cleanBtn");
         this._cleanBtn.tipData = LanguageMgr.GetTranslation("drgnBoat.game.cleanBtnTipTxt");
         addChild(this._bg);
         ...
         for(i = 0; i < 4; i++)
         {
            tmp = ComponentFactory.Instance.creat("drgnBoat.freeTipMc") as MovieClip;
            tmp.x = -36;
            tmp.y = -14 + 44 * i;
      ...
      private function initEvent() : void
      {
         ...
         DrgnBoatManager.instance.addEventListener(DrgnBoatManager.REFRESH_ITEM_FREE_COUNT,this.refreshFreeCount);
      }
      
      private function refreshFreeCount(event:Event) : void
      {
         var tmp:Array = DrgnBoatManager.instance.itemFreeCountList;
      ...
      {
         var tmp:DrgnBoatEvent = new DrgnBoatEvent(DrgnBoatManager.LEAP_PROMPT_SHOW_HIDE);
         tmp.data = {"isShow":false};
         DrgnBoatManager.instance.dispatchEvent(tmp);
      }
      ...
      public function useSkill() : void
      {
         ...
         {
            this.sendUseSkillSocket(this._recordClickTag,false,true,this.targetId,this.targetZone);
            this.unEnableBtn(this._recordClickTag);
            return;
         }
         var tmpObj:Object = DrgnBoatManager.instance.getBuyRecordStatus(this._recordClickTag + 2);
         var needMoney:int = int(DrgnBoatManager.instance.dataInfo.useSkillNeedMoney[this._recordClickTag]);
         if(Boolean(tmpObj.isNoPrompt))
         {
            ...
            else
            {
               ...
               {
                  this.sendUseSkillSocket(this._recordClickTag,tmpObj.isBand,this._freeTipList[this._recordClickTag].visible,this.targetId,this.targetZone);
                  this.unEnableBtn(this._recordClickTag);
                  return;
               }
         ...
         var confirmFrame:BaseAlerFrame = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("AlertDialog.Info"),LanguageMgr.GetTranslation("drgnBoat.frame.useSkillConfirmTxt" + this._recordClickTag,needMoney),LanguageMgr.GetTranslation("ok"),LanguageMgr.GetTranslation("cancel"),true,true,true,LayerManager.BLCAK_BLOCKGOUND,null,"DrgnBoatBuyConfirmView1",30,true,AlertManager.SELECTBTN);
      ...
      private function useSkillConfirm(evt:FrameEvent) : void
      {
         ...
         {
            needMoney = int(DrgnBoatManager.instance.dataInfo.useSkillNeedMoney[this._recordClickTag]);
            ...
            {
               tmpObj = DrgnBoatManager.instance.getBuyRecordStatus(this._recordClickTag + 2);
               tmpObj.isNoPrompt = true;
               tmpObj.isBand = confirmFrame.isBand;
            }
            this.sendUseSkillSocket(this._recordClickTag,confirmFrame.isBand,this._freeTipList[this._recordClickTag].visible,this.targetId,this.targetZone);
            this.unEnableBtn(this._recordClickTag);
         }
      }
      ...
      private function useSkillReConfirm(evt:FrameEvent) : void
      {
         ...
         if(evt.responseCode == FrameEvent.SUBMIT_CLICK || evt.responseCode == FrameEvent.ENTER_CLICK)
         {
            ...
            this.sendUseSkillSocket(this._recordClickTag,false,this._freeTipList[this._recordClickTag].visible,this.targetId,this.targetZone);
            this.unEnableBtn(this._recordClickTag);
         }
      }
      ...
      private function sendUseSkillSocket(type:int, isBand:Boolean, isFree:Boolean, otherId:int = 0, otherZone:int = -1) : void
      {
         ...
         else
         {
            ...
            arr.push(otherId);
            arr.push(otherZone);
            DrgnBoatManager.instance.missileArgArr = arr;
      ...
      private function removeEvent() : void
      {
         ...
         DrgnBoatManager.instance.removeEventListener(DrgnBoatManager.REFRESH_ITEM_FREE_COUNT,this.refreshFreeCount);
      }
