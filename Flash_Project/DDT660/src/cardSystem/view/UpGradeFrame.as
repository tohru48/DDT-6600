package cardSystem.view
{
   ...
   public class UpGradeFrame extends Frame
   {
      ...
      private function __upDateHandler(event:DictionaryEvent) : void
      {
         ...
         if(cInfo.TemplateID != this._cardInfo.TemplateID)
         {
            return;
         }
         if(cInfo.CardGP - this._lastGP == 0)
         {
            return;
         }
         MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("ddt.cardSystem.upgradeFrame.upSuccess",cInfo.CardGP - this._lastGP),0,true);
         this.cardInfo = cInfo;
      ...
      public function set cardInfo(value:CardInfo) : void
      {
         ...
         for(var i:int = 0; i < this._upgradeRuleArr.length; i++)
         {
            if((this._upgradeRuleArr[i] as SetsUpgradeRuleInfo).Level == value.Level + 1)
            {
               this._ruleInfo = this._upgradeRuleArr[i];
               break;
            }
         }
         this.upView();
