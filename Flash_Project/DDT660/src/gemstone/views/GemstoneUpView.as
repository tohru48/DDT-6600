package gemstone.views
{
   ...
   public class GemstoneUpView extends Frame
   {
      ...
      private var _dataList:Vector.<InventoryItemInfo> = new Vector.<InventoryItemInfo>();
      ...
      private function initView() : void
      {
         ...
         info = this._selfInfo == null ? null : this._selfInfo.Bag.getItemAt(GemstoneManager.HariPPLACE);
         ...
         this._gemstoneCriView.upDataIcon(info);
         ...
         this._decorateBtn = new FTextButton("gemstone.yellow2","gemstoneBtnTxt1");
         this._decorateBtn.width = 80;
         ...
         this._bagitem = new BagCell(0,stoneInfo);
         this._bagitem.x = 32;
         this._bagitem.y = 297;
      ...
      public function upDatafitCount() : void
      {
         ...
         conut += bagInfo.getItemCountByTemplateId(201264);
         this._bagitem.setCount(conut);
      }
      
      private function visibleGroup1(bool:Boolean) : void
      {
         this._mouseClick.visible = bool;
         this._selectTxt.visible = bool;
         this._gemstoneInfoView.visible = bool;
      }
      
      private function visibleGroup2(bool:Boolean) : void
      {
         this._txt1.visible = bool;
         this._txt2.visible = bool;
         this._txt4.visible = bool;
         this._expBar.visible = bool;
         this._selectBtn.visible = bool;
         this._gemstoneInfoView.visible = bool;
      }
      ...
      protected function btnClickHander(event:MouseEvent) : void
      {
         ...
         var gemstoneList:Vector.<GemstListInfo> = null;
         var str:String = null;
         SoundManager.instance.play("008");
         this.visibleGroup1(false);
         this.visibleGroup2(true);
         ...
         {
            info = this._selfInfo == null ? null : this._selfInfo.Bag.getItemAt(GemstoneManager.FacePLACE);
         ...
         {
            info = this._selfInfo == null ? null : this._selfInfo.Bag.getItemAt(GemstoneManager.GlassPPLACE);
         ...
         {
            info = this._selfInfo == null ? null : this._selfInfo.Bag.getItemAt(GemstoneManager.SuitPLACE);
         ...
         {
            info = this._selfInfo == null ? null : this._selfInfo.Bag.getItemAt(GemstoneManager.DecorationPLACE);
         ...
         this._gemstoneCriView.initFigSkin(str);
         this._gemstoneCriView.upDataIcon(info);
         if(Boolean(gemstoneList))
         {
            this._gemstoneCriView.resetGemstoneList(gemstoneList);
         }
         this._seletedBitMap.x = event.currentTarget.x - 2;
         this._seletedBitMap.y = event.currentTarget.y - 2;
         this._seletedBitMap.width = event.currentTarget.width + 3;
      }
      ...
      private function mouseClickHander(event:MouseEvent) : void
      {
         ...
         var allCount:int = bagInfo.getItemCountByTemplateId(templeteId) + bagInfo.getItemCountByTemplateId(100100);
         if(allCount <= 0)
         {
            MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("ddt.gemstone.goodsNotNum"));
            return;
         }
         if(infoCount > 0)
         {
            this.sendFigSpiritUpGrade(goodsId,templeteId);
         }
         if(!(infoCount > 0 && this._isAutoSele == 0))
         {
            goodsId = 10010001;
            templeteId = 100100;
            infoCount += bagInfo.getItemCountByTemplateId(templeteId);
            if(infoCount > 0)
            {
               this.sendFigSpiritUpGrade(goodsId,templeteId);
            }
      ...
      private function sendFigSpiritUpGrade($goodsId:int, $templeteId:int) : void
      {
         ...
         sendInfo.autoBuyId = this._isAutoSele;
         sendInfo.goodsId = $goodsId;
         sendInfo.type = 1;
         sendInfo.templeteId = $templeteId;
         sendInfo.fightSpiritId = this._fightSpiritId;
         sendInfo.equipPlayce = this._equipPlayce;
         sendInfo.place = FIG_POS;
         sendInfo.count = 1;
      ...
      {
         this._gemstoneCriView.upGradeAction(info);
      }
