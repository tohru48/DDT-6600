package catchInsect.view
{
   ...
   public class CatchInsectScneneMap extends Sprite implements Disposeable
   {
      ...
      {
         this._model.addEventListener(CatchInsectRoomEvent.PLAYER_NAME_VISIBLE,this.menuChange);
         this._model.addEventListener(CatchInsectRoomEvent.PLAYER_CHATBALL_VISIBLE,this.menuChange);
      ...
      private function __removeMonster(pEvent:DictionaryEvent) : void
      {
         ...
         var monster:CatchInsectMonster = this._monsters[monsterInfo.ID] as CatchInsectMonster;
      ...
      private function menuChange(evt:CatchInsectRoomEvent) : void
      {
         switch(evt.type)
         {
            case CatchInsectRoomEvent.PLAYER_NAME_VISIBLE:
               this.nameVisible();
         }
      }
      ...
      {
         var christmasRoomPlayer:CatchInsectRoomPlayer = null;
      ...
      {
         var player:CatchInsectRoomPlayer = null;
      ...
      protected function __click(event:MouseEvent) : void
      {
         ...
         this.autoMove = false;
         if(new Date().getTime() - this._lastClick > this._clickInterval)
         {
      ...
      public function movePlayer(id:int, p:Array) : void
      {
         ...
         {
            christmasRoomPlayer = this._characters[id] as CatchInsectRoomPlayer;
      ...
      {
         var roomPlayer:CatchInsectRoomPlayer = null;
         if(Boolean(this._characters[id]))
         {
            roomPlayer = this._characters[id] as CatchInsectRoomPlayer;
            if(stauts == 0)
            {
            ...
            else if(stauts == 1)
            {
               ...
               roomPlayer.addEventListener(CatchInsectRoomEvent.READYFIGHT,this.__otherPlayrStartFight);
      ...
      public function __otherPlayrStartFight(evt:CatchInsectRoomEvent) : void
      {
         ...
         roomPlayer.removeEventListener(CatchInsectRoomEvent.READYFIGHT,this.__otherPlayrStartFight);
         roomPlayer.sceneCharacterDirection = SceneCharacterDirection.getDirection(roomPlayer.playerPoint,this.armyPos);
         roomPlayer.dispatchEvent(new SceneCharacterEvent(SceneCharacterEvent.CHARACTER_DIRECTION_CHANGE,false));
      ...
      public function updateSelfStatus(value:int) : void
      {
         if(!this.selfPlayer)
         {
            this._isUpdateComplete = false;
            this._updateState = value;
            return;
         }
         this._isUpdateComplete = true;
         if(this.selfPlayer.playerVO.playerStauts == 2)
         {
            this.selfPlayer.playerVO.playerPos = CatchInsectMananger.instance.catchInsectInfo.playerDefaultPos;
      ...
      public function setCenter(event:SceneCharacterEvent = null) : void
      {
         ...
         {
            xf = -(CatchInsectMananger.instance.catchInsectInfo.playerDefaultPos.x - MoonSceneMap.GAME_WIDTH / 2);
            yf = -(CatchInsectMananger.instance.catchInsectInfo.playerDefaultPos.y - MoonSceneMap.GAME_HEIGHT / 2) + 50;
         }
         if(xf > 0)
         {
            xf = 0;
      ...
      public function addSelfPlayer() : void
      {
         ...
         if(!this.selfPlayer)
         {
            selfPlayerVO = CatchInsectMananger.instance.catchInsectInfo.myPlayerVO;
            selfPlayerVO.playerInfo = PlayerManager.Instance.Self;
            this._currentLoadingPlayer = new CatchInsectRoomPlayer(selfPlayerVO,this.addPlayerCallBack);
         }
      }
      ...
      protected function __addPlayer(event:DictionaryEvent) : void
      {
         ...
         this._currentLoadingPlayer = new CatchInsectRoomPlayer(playerVO,this.addPlayerCallBack);
      }
      ...
      private function addPlayerCallBack(roomPlayer:CatchInsectRoomPlayer, isLoadSucceed:Boolean, vFlag:int) : void
      {
         if(vFlag == 0)
         {
            if(!this.articleLayer || !roomPlayer)
            {
               return;
            }
            this._currentLoadingPlayer = null;
            roomPlayer.sceneScene = this.sceneScene;
            roomPlayer.setSceneCharacterDirectionDefault = roomPlayer.sceneCharacterDirection = roomPlayer.playerVO.scenePlayerDirection;
            if(!this.selfPlayer && roomPlayer.playerVO.playerInfo.ID == PlayerManager.Instance.Self.ID)
            {
               roomPlayer.playerVO.playerPos = roomPlayer.playerVO.playerPos;
               this.selfPlayer = roomPlayer;
            ...
            {
               this.articleLayer.addChild(roomPlayer);
            }
            roomPlayer.playerPoint = roomPlayer.playerVO.playerPos;
            roomPlayer.sceneCharacterStateType = "natural";
            this._characters.add(roomPlayer.playerVO.playerInfo.ID,roomPlayer);
            roomPlayer.isShowName = this._model.playerNameVisible;
            if(!this._isUpdateComplete)
            {
               this.updateSelfStatus(this._updateState);
            }
         }
      }
      ...
      {
         var mon:CatchInsectMonster = null;
         var pos:Point = null;
         ...
         if(type == "naturalStandFront" || type == "naturalStandBack")
         {
            ...
            if(Boolean(mon) && mon.MonsterState <= InsectInfo.LIVIN)
            {
      ...
      protected function __removePlayer(event:DictionaryEvent) : void
      {
         ...
         var player:CatchInsectRoomPlayer = this._characters[id] as CatchInsectRoomPlayer;
      ...
      {
         this._model.removeEventListener(CatchInsectRoomEvent.PLAYER_NAME_VISIBLE,this.menuChange);
         this._model.removeEventListener(CatchInsectRoomEvent.PLAYER_CHATBALL_VISIBLE,this.menuChange);
      ...
      {
         var p:CatchInsectRoomPlayer = null;
         var o:CatchInsectMonster = null;
         var i:int = 0;
         var player:CatchInsectRoomPlayer = null;
         this.removeEvent();
         ...
         if(Boolean(this.articleLayer))
         {
            ...
            {
               player = this.articleLayer.getChildAt(i - 1) as CatchInsectRoomPlayer;
               if(Boolean(player))
               {
                  player.removeEventListener(SceneCharacterEvent.CHARACTER_MOVEMENT,this.setCenter);
                  player.removeEventListener(SceneCharacterEvent.CHARACTER_ACTION_CHANGE,this.playerActionChange);
