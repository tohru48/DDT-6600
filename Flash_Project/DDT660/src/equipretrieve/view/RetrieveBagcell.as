package equipretrieve.view
{
   ...
   public class RetrieveBagcell extends BagCell
   {
         ...
         if(effect.data is InventoryItemInfo)
         {
            ...
            else
            {
               if(_bagType == 11 || info.BagType == 11)
               {
                  ...
                  {
                     SocketManager.Instance.out.sendMoveGoods(BagInfo.CONSORTIA,info.Place,_bagType,place,info.Count);
                     effect.action = DragEffect.NONE;
                  }
      ...
      {
         if(Boolean(info) && (info.CategoryID == 10 || info.CategoryID == 11 || info.CategoryID == 12))
         {
