package ddt.manager
{
   ...
   public class DropGoodsManager implements Disposeable
   {
      ...
      public static function play(item:Array, begin:Point = null, end:Point = null, isEmailAniam:Boolean = false) : void
      {
         ...
         for(var i:int = 0; i < dgm.goodsList.length; i++)
         {
            itemInfo = dgm.goodsList[i];
            cell = new BaseCell(new Sprite(),itemInfo);
            cell.setContentSize(48,48);
            timeOutId = setTimeout(dgm.packUp,200 + i * 200,cell,dgm.onCompletePackUp);
      ...
      private function packUp(goods:DisplayObject, callBack:Function) : void
      {
         ...
         goods.x = this.beginPoint.x;
         goods.y = this.beginPoint.y;
         goods.alpha = 0.5;
         goods.scaleX = 0.85;
         goods.scaleY = 0.85;
         this.parentContainer.addChild(goods);
         var p4:Point = this.endPoint;
         var p2:Point = new Point(this.beginPoint.x - (this.beginPoint.x - p4.x) / 2,this.beginPoint.y - 100);
         var p1:Point = new Point(p2.x - (p2.x - this.beginPoint.x) / 2,this.beginPoint.y - 60);
         var p3:Point = new Point(p4.x - (p4.x - p2.x) / 2,this.beginPoint.y + 30);
         ...
         this.tweenArr.push(timeline);
      }
      
      private function onCompletePackUp(goods:DisplayObject) : void
      {
         var emailMc:MovieClipWrapper = null;
         var bagMc:MovieClipWrapper = null;
         if(Boolean(goods) && this.parentContainer.contains(goods))
         {
            ObjectUtils.disposeObject(goods);
            goods = null;
         }
         if(_isEmailAniam)
         {
            emailMc = this.getEmailAniam();
            if(Boolean(emailMc.movie))
            {
               this.parentContainer.addChild(emailMc.movie);
            }
         }
         else
         {
            bagMc = this.getBagAniam();
            if(Boolean(bagMc.movie))
            {
               this.parentContainer.addChild(bagMc.movie);
            }
         }
      ...
      private function getBagAniam() : MovieClipWrapper
      {
         ...
         mc = ClassUtils.CreatInstance("asset.toolbar.bagMCAsset") as MovieClip;
      ...
      private function getEmailAniam() : MovieClipWrapper
      {
         ...
         pt = ComponentFactory.Instance.creatCustomObject("dropGoods.emailPoint");
         mc.x = pt.x;
         mc.y = pt.y;
         return new MovieClipWrapper(mc,true,true);
      }
      ...
      private function setGoodsList(item:Array) : void
      {
         ...
         for(i = 0; i < item.length; i++)
         {
            ...
            while(true)
            {
               ...
               if(item[i].TemplateID == this.goodsList[j].TemplateID)
               {
                  break;
               }
               j++;
         ...
         if(itemList.length > 7)
         {
            ...
            {
               this.goodsList.push(itemList[i].item);
               i++;
            }
         }
         else
