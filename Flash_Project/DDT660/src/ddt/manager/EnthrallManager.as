package ddt.manager
{
   ...
   public class EnthrallManager
   {
      ...
      private function changeCIDChecker(evt:CrazyTankSocketEvent) : void
      {
         ...
         this._popCIDChecker = pkg.readBoolean();
         if(this._popCIDChecker)
         {
            this._timer1.start();
         }
      ...
      {
         var pkg:PackageIn = evt.pkg;
         this._enthrallSwicth = pkg.readBoolean();
         this._interfaceID = pkg.readInt();
         this._hasApproved = pkg.readBoolean();
         this._isMinor = pkg.readBoolean();
