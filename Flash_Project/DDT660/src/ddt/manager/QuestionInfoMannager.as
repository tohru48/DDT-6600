package ddt.manager
{
   ...
   public class QuestionInfoMannager
   {
      ...
      public function setup(analyze:QuestionInfoAnalyze) : void
      {
         this._allQuestion = analyze.allQuestion;
         SocketManager.Instance.addEventListener(CrazyTankSocketEvent.ANSWERBOX_QUESTIN,this.__AnswerBoxQuestion);
      }
      
      private function __AnswerBoxQuestion(event:CrazyTankSocketEvent) : void
      {
         this._questionCatalogID = event.pkg.readInt();
         this._questionID = event.pkg.readInt();
         this._questionTotal = event.pkg.readInt();
         this._questionCurrentNum = event.pkg.readInt();
         this._correctQuestionNum = event.pkg.readInt();
