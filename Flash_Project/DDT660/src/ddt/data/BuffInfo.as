   ...
   public class BuffInfo
   {
      ...
      public function calculatePayBuffValidDay() : void
      {
         ...
         if(Boolean(this.BeginData))
         {
            ...
            begin = new Date(this.BeginData.fullYear,this.BeginData.month,this.BeginData.date);
            now = new Date(now.fullYear,now.month,now.date);
            elapsed = (now.time - begin.time) / TimeManager.DAY_TICKS;
            if(elapsed < this.ValidDate)
            {
               this._valided = true;
               this.day = this.ValidDate - elapsed;
            }
            else
            {
               this._valided = false;
      ...
      private function getLeftTime() : Number
      {
         ...
         if(this.IsExist)
         {
            leftTime = this.ValidDate - Math.floor((TimeManager.Instance.Now().time - this.BeginData.time) / TimeManager.Minute_TICKS);
         }
         else
         {
            leftTime = -1;
      ...
      public function initItemInfo() : void
      {
         switch(this.Type)
         {
            ...
            case LABYRINTH_BUFF:
               this._buffItem = new ItemTemplateInfo();
               break;
            case DOUBLE_CONTRIBUTE:
               this._buffItem = ItemManager.Instance.getTemplateById(EquipType.DOUBLE_CONTRIBUTE);
               break;
            default:
