package ddt.data.map
{
   ...
   public class MissionInfo extends EventDispatcher
   {
      ...
      public function findMissionOverInfo(playerid:int) : BaseSettleInfo
      {
         ...
         for(var i:int = 0; i < this.missionOverPlayer.length; i++)
         {
            if(this.missionOverPlayer[i].playerid == playerid)
            {
               return this.missionOverPlayer[i];
            }
         }
         return null;
      }
