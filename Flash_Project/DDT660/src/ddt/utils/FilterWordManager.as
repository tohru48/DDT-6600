package ddt.utils
{
   ...
   public class FilterWordManager
   {
      ...
      public static function isGotForbiddenWords(str:String, level:String = "chat") : Boolean
      {
         ...
         if(level == FILTER_TYPE_NAME || level == FILTER_TYPE_CHAT || level == FILTER_TYPE_ALL)
         {
            for(i = 0; i < count; i++)
            {
      ...
      private static function formatChannelWords(str:String) : String
      {
         ...
         if(isGotChannelWord && Boolean(str))
         {
            return str;
         }
         return undefined;
      ...
      {
         var s:String = null;
         var startIdx:int = int(obj["idx"]);
         var len:int = int(obj["length"]);
         ...
         return str.replace(s,getXXX(len));
      }
      
      public static function filterWrod(s:String) : String
      {
         var re_str1:String = null;
      ...
      {
         if(isGotForbiddenWords(str,FILTER_TYPE_SERVER))
         {
      ...
      private static function getXXX(len:int) : String
      {
         ...
         return REPLACEWORD.slice(startIdx,startIdx + len);
      }
   }
}
