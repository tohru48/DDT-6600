package ddt.view.tips
{
   ...
   public class LevelPicCreater
   {
      ...
      public static function creatLelvePic(level:int) : Sprite
      {
         ...
         if(level < 10)
         {
            result.addChild(ComponentFactory.Instance.creatBitmap(pathTip + LEVELTIPCLASSES[level]));
         }
         else if(level > 9)
         {
            units = int(level / 10);
            tens = level % 10;
            ub = ComponentFactory.Instance.creatBitmap(pathTip + LEVELTIPCLASSES[units]);
            tb = ComponentFactory.Instance.creatBitmap(pathTip + LEVELTIPCLASSES[tens]);
            tb.x = ub.width;
            result.addChild(ub);
            result.addChild(tb);
         }
      ...
      {
         var _tmp:uint = 0;
         var _lt1:Bitmap = null;
         var _lt2:Bitmap = null;
         if(_level > 9)
         {
            _tmp = Math.floor(_level / 10);
            _lt1 = ComponentFactory.Instance.creat(pathTip + LEVELTIPCLASSES[_tmp]);
            _lt1.x = dx - 4;
            _lt1.y = dy;
            parentObj.addChild(_lt1);
            _tmp = _level % 10;
            _lt2 = ComponentFactory.Instance.creat(pathTip + LEVELTIPCLASSES[_tmp]);
