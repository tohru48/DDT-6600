package ddt.view.tips
{
   ...
   public class PlayerTip extends Sprite implements Disposeable
   {
      ...
      public function show(yOffset:int) : void
      {
         ...
         x = pos.x - this._bg.width;
         y = yOffset - this._bg.height - (this._bottomBtnsContainer.visible ? this._bottomBg.height : 0);
         if(x < X_MARGINAL)
         {
            x = X_MARGINAL;
         }
      ...
      public function update() : void
      {
         ...
         if(Boolean(this._info))
         {
            ...
            if(tInfo.IsVIP)
            {
               ...
               this._vipName = VipController.instance.getVipNameTxt(138,tInfo.typeVIP);
            ...
            this._clubTxt.text = LanguageMgr.GetTranslation("tank.menu.ClubName") + "：" + (Boolean(tInfo.ConsortiaName) ? tInfo.ConsortiaName : "");
         }
         else
         {
         ...
         else
         {
            ...
            {
               this._btnInvite.visible = ConsortiaDutyManager.GetRight(PlayerManager.Instance.Self.Right,ConsortiaDutyType._2_Invite) && tInfo.ConsortiaID == 0;
               this._bottomBtnsContainer.visible = this._btnInvite.visible;
            }
         }
         if((tInfo.apprenticeshipState == AcademyManager.NONE_STATE || tInfo.apprenticeshipState == AcademyManager.MASTER_STATE) && tInfo.ID != PlayerManager.Instance.Self.ID && tInfo.ID != PlayerManager.Instance.Self.masterID && tInfo.Grade >= AcademyManager.TARGET_PLAYER_MIN_LEVEL)
         {
         ...
         if(PlayerManager.isShowPHP)
         {
            ...
            this._photo.userID = String(tInfo.LoginName);
         }
      }
         ...
         if(Boolean(this._info))
         {
            switch(event.currentTarget)
            {
               ...
               case this._btnExpel:
                  ...
                  alert.addEventListener(FrameEvent.RESPONSE,this.__frameEventHandler);
                  break;
               ...
               case this.btnChallenge:
                  IMController.Instance.dispatchEvent(new Event(CHALLENGE));
                  break;
               ...
               case this._btnViewInfo:
                  this.hide();
                  PlayerInfoViewControl.viewByID(this._info.ID,-1,true,false);
      ...
      {
         if(Boolean(e.changedProperties["DutyLevel"]))
         {
