package ddt.view
{
   ...
   public class SNSFrame extends BaseAlerFrame
   {
      ...
      private function sendDynamic() : void
      {
         ...
         uv.typeId = this.typeId;
         uv.playerTest = this._getStr();
         switch(uv.typeId)
         {
            case 6:
            case 7:
         ...
         uv.inviteCaption = this.backgroundServerTxt;
         if(Boolean(this._inputText))
         {
            uv.playerTest = this._inputText.text;
         }
         uv.ran = Math.random();
         SocketManager.Instance.out.sendSnsMsg(this.typeId);
         var load:BaseLoader = LoadResourceManager.Instance.createLoader(PathManager.getSnsPath(),BaseLoader.REQUEST_LOADER,uv);
