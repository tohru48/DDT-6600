package ddt.view.chat
{
   ...
   public class ChatOutputView extends Sprite
   {
      ...
      public function set isLock(value:Boolean) : void
      {
         ...
         this.setChannelBtnVisible(!value);
         this.setBgVisible(!value);
      ...
      {
         this._lockBtnInGame.enable = this._lockBtn.enable = value;
      }
      ...
      private function __leftBtnsClick(e:MouseEvent) : void
      {
         ...
         switch(e.currentTarget)
         {
            ...
            case this._lockBtnInGame:
               ...
               {
                  ChatManager.HALL_CHAT_LOCK = this.isLock;
               }
      ...
      private function __onAddChat(event:ChatEvent) : void
      {
         ...
         if(addedChatData.channel == ChatInputView.PRIVATE && addedChatData.sender != PlayerManager.Instance.Self.NickName && this._channel != CHAT_OUPUT_PRIVATE && this._channel != CHAT_OUPUT_CURRENT)
         {
            this._privateBtnEffect.play();
         }
         if(this._model.getInputInOutputChannel(addedChatData.channel,this._channel))
         {
      ...
      private function initBtns() : void
      {
         ...
         this._privateBtnEffect = EffectManager.Instance.creatEffect(EffectTypes.ADD_MOVIE_EFFECT,this._privateBtn,"chat.ChatPrivateShineEffect",orginPos,privateEffectPos);
      ...
      {
         if(_val == this._more.selected)
         {
            return;
         }
         this._more.selected = _val;
         this.updateOutputViewState();
      }
      ...
      public function updateCurrnetChannel() : void
      {
         ...
         if(this.isInGame())
         {
            result = this._model.getChatsByOutputChannel(this._channel,this._currentOffset,5);
         }
         else
         {
            result = this._model.getChatsByOutputChannel(this._channel,this._currentOffset,this._more.selected ? 16 : 6);
         ...
         this._currentOffset = result["offset"];
         this._outputField.setChats(result["result"]);
         this.goBottom();
         this.updateShine();
