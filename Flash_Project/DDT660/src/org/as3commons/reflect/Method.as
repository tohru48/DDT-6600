   ...
   public class Method extends MetadataContainer implements INamespaceOwner
   {
      ...
      public function get fullName() : String
      {
         ...
         if(this.updateFullName)
         {
            ...
            if(this.isStatic)
            {
               this._fullName += "static ";
            }
            this._fullName += this.name + "(";
            len = int(this._parameters.length);
            for(i = 0; i < len; i++)
            {
               p = this._parameters[i];
               this._fullName += p.type.name;
               this._fullName += i < len - 1 ? ", " : "";
            }
            this._fullName += "):" + this.returnType.name;
      ...
      public function get parameters() : Array
      {
         ...
         var result:Array = [];
         var len:int = int(this._parameters.length);
