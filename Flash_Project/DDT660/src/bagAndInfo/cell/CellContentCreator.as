   ...
   public class CellContentCreator extends Sprite implements Disposeable
   {
      ...
      public function loadSync(callBack:Function) : void
      {
         ...
         else
         {
            ...
            {
               color = EquipType.isEditable(this._info) && InventoryItemInfo(this._info).Color != null ? InventoryItemInfo(this._info).Color : "";
               this._loader = this._factory.createLayer(this._info,this._info.NeedSex == 1,color,BaseLayer.ICON);
            }
            else
            {
      ...
      public function setColor(color:*) : Boolean
      {
         if(this._loader != null)
         {
            return this._loader.setColor(color);
         }
         return false;
      }
