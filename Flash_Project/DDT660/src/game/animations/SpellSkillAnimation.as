package game.animations
{
   ...
   public class SpellSkillAnimation extends EventDispatcher implements IAnimate
   {
      ...
      public function SpellSkillAnimation(x:Number, y:Number, stageWidth:Number, stageHeight:Number, mapWidth:Number, mapHeight:Number, player:GamePlayer, gameview:GameViewBase)
      {
         super();
         this._scale = 1.5;
         var _minX:Number = -mapWidth * this._scale + stageWidth;
         var _minY:Number = -mapHeight * this._scale + stageHeight;
         var m:Matrix = new Matrix(this._scale,0,0,this._scale);
         this._end = new Point(x,y);
         this._end = m.transformPoint(this._end);
         this._end.x = stageWidth / 2 - this._end.x;
         this._end.y = stageHeight / 4 * 3 - this._end.y;
         this._end.x = this._end.x > 0 ? 0 : (this._end.x < _minX ? _minX : this._end.x);
         this._end.y = this._end.y > 0 ? 0 : (this._end.y < _minY ? _minY : this._end.y);
         this._player = player;
