package game.view
{
   ...
   public class DungeonHelpView extends Sprite implements Disposeable
   {
      ...
      private function __timerComplete(evt:TimerEvent) : void
      {
         ...
         if(Boolean(this._turnButton) && Boolean(this._gameContainer))
         {
            bounds = this._turnButton.getBounds(this._gameContainer);
            bounds.x = 860;
            bounds.y = 5;
         }
         this.close(bounds);
      ...
      {
         var winArr:Array = null;
         var lostArr:Array = null;
         if(GameManager.Instance.Current.missionInfo != null)
         {
            winArr = GameManager.Instance.Current.missionInfo.success.split("<br>");
            lostArr = GameManager.Instance.Current.missionInfo.failure.split("<br>");
            this._winTxt1.text = winArr[0];
            this._arrow1.y = this._winTxt1.y + 5;
            this._arrow1.x = this._winTxt1.x - 15;
            this._arrow1.visible = true;
            if(winArr.length >= 2)
            {
               this._winTxt2.text = winArr[1];
      ...
      private function __closeHandler(evt:MouseEvent) : void
      {
         ...
         {
            bounds = this._barrier.getBounds(this._gameContainer);
            bounds.height = 1;
            bounds.width = 1;
            this.close(bounds);
         }
