package game.view.tool
{
   ...
   public class PowerStrip extends Sprite implements Disposeable
   {
      ...
      public function PowerStrip()
      {
         ...
         g.beginFill(0);
         g.drawRect(0,0,this._bg.width,this._bg.height);
         g.endFill();
         ...
         this._tipSprite.addChild(tipBitm);
         tipBitm.alpha = 0;
      ...
      private function __update(evt:LivingEvent) : void
      {
         ...
         var energy:Number = Math.round(this._self.energy);
         this._powerStripTipInfo.current = energy;
         this._powerStripTipInfo.total = this._self.maxEnergy;
         this._powerStripTip.tipData = this._powerStripTipInfo;
         this._powerField.text = String(energy);
         ...
         {
            this.canCount = false;
            NewHandContainer.Instance.showArrow(ArrowType.POWER_STRIP,-30,"trainer.powerStripArrowPos","asset.trainer.txtPowerStrip1","trainer.powerStripTipPos",null,6000);
