package game.view.tool
{
   ...
   public class BloodStrip extends Sprite implements Disposeable, ITipedDisplay
   {
      ...
      private function update(blood:int, maxBlood:int) : void
      {
         ...
         {
            if(this._info.current > blood)
            {
            }
            this._info.current = blood < 0 ? 0 : blood;
            this._info.total = maxBlood;
            this.tipData = this._info;
         }
         if(blood < 0)
         {
         ...
         this._mask.width = this._HPStrip.width * (blood / maxBlood);
         this._mask.x = this._HPStrip.width - this._mask.width;
         this._HPTxt.text = String(blood);
      }
