package game.view.propertyWaterBuff
{
   ...
   public class PropertyWaterBuffTip extends Sprite implements Disposeable, ITip
   {
      ...
      {
         this._buffInfo = data as BuffInfo;
         this.update();
      }
      
      private function update() : void
      {
         var day:int = this._buffInfo.getLeftTimeByUnit(TimeManager.DAY_TICKS);
         var hour:int = this._buffInfo.getLeftTimeByUnit(TimeManager.HOUR_TICKS);
         var min:int = this._buffInfo.getLeftTimeByUnit(TimeManager.Minute_TICKS);
         this._name.text = this._buffInfo.buffName;
         this._explication.text = this._buffInfo.buffItemInfo.Description;
         if(day > 0)
         {
            this._timer.text = LanguageMgr.GetTranslation("game.view.propertyWaterBuff.timer",day);
         }
         else if(hour > 0)
         {
            this._timer.text = LanguageMgr.GetTranslation("game.view.propertyWaterBuff.timerI",hour);
         }
         else if(min > 0)
         {
            this._timer.text = LanguageMgr.GetTranslation("game.view.propertyWaterBuff.timerII",min);
         }
         else
         {
