   ...
   public class GhostMoveAction extends BaseAction
   {
      ...
      override public function execute() : void
      {
         ...
         if(Point.distance(this._startPos,this._target) > this._vp.length)
         {
            ...
            {
               (this._player as GameLocalPlayer).localPlayer.energy -= Math.round(Point.distance(posA,posB) / 1.5);
            }
         }
         else
         {
         ...
         this._life += 40;
         for each(act in this._pickBoxActions)
         {
            if(this._life >= act.time && !act.executed)
            {
               act.execute(this._player);
            }
         }
      }
