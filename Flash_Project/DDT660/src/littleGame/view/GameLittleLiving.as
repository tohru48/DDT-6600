package littleGame.view
{
   ...
   public class GameLittleLiving extends Sprite implements Disposeable
   {
      ...
      protected function createBody() : void
      {
         ...
         ch = CharacterFactory.Instance.creatChacrater(this._living.ddt_internal::_modelID);
      ...
      protected function __directionChanged(event:LittleLivingEvent) : void
      {
         ...
         {
            if(this._living.isPlayer)
            {
               if(!(this._body as LittleGameCharacter).isComplete)
               {
                  this._body.scaleX = 1;
               }
               else if(this._living.direction == DirectionType.LEFT_DOWN || this._living.direction == DirectionType.RIGHT_UP)
               {
                  this._body.scaleX = 1;
               }
               else
      ...
      protected function centerBody() : void
      {
         ...
         if(Boolean(this._body) && Boolean(ch))
         {
            this._body.x = this._body.scaleX == 1 ? -ch.registerPoint.x : Number(ch.registerPoint.x);
         }
      }
