package littleGame.view
{
   ...
   public class GameScene extends Sprite implements Disposeable
   {
      ...
      private var selfPos:Point = new Point();
      
      private var otherPos:Point = new Point();
      ...
      private function drawLivings() : void
      {
         ...
         for each(living in livings)
         {
            ...
            {
               setTimeout(this.drawLiving,150 * i++,living);
            }
         }
      }
      ...
      private function drawLiving(living:LittleLiving) : GameLittleLiving
      {
         ...
         if(this._game == null || this._game.livings == null || this._game.livings[living.id] != living)
         {
            return null;
         }
         if(living.isSelf)
         ...
         if(Boolean(this._gameLivings[living.id]))
         {
         }
      ...
      private function drawGrid() : void
      {
         ...
         var rows:int = int(nodes.length);
         for(var i:int = 0; i < rows; i++)
         {
            cols = int(nodes[i].length);
            for(j = 0; j < cols; j++)
            {
