package littleGame.view
{
   ...
   public class PriceShape extends Bitmap implements Disposeable
   {
      ...
      {
         var back:BitmapData = LittleGameManager.Instance.Current.ddt_internal::priceBack;
         var left:int = back.width;
         var src:BitmapData = LittleGameManager.Instance.Current.ddt_internal::priceNum;
         var priceStr:String = price.toString();
         var w:int = src.width / 10;
         var h:int = src.height;
         var bitmap:BitmapData = new BitmapData(left + priceStr.length * w,back.height + 13,true,0);
         ...
         for(var i:int = 0; i < len; i++)
         {
            ...
            bitmap.copyPixels(src,rect,new Point(left + w * i,13));
         }
