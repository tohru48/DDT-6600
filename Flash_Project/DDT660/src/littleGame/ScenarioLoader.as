package littleGame
{
   ...
   public class ScenarioLoader extends EventDispatcher implements IProcessObject
   {
      ...
      private function loadObjects() : void
      {
         ...
         {
            loader = LoadResourceManager.Instance.createLoader(PathManager.solveLittleGameObjectPath(objects[i]),BaseLoader.MODULE_LOADER);
      ...
      {
         this._resLoader = LoadResourceManager.Instance.createLoader(PathManager.solveLittleGameResPath(this._scene.id),BaseLoader.MODULE_LOADER);
         ...
         LoadResourceManager.Instance.startLoad(this._resLoader);
      }
      ...
      private function loadConfig() : void
      {
         ...
         this._configLoader = LoadResourceManager.Instance.createLoader(PathManager.solveLittleGameConfigPath(this._scene.id),BaseLoader.BYTE_LOADER,args);
         ...
         LoadResourceManager.Instance.startLoad(this._configLoader);
      }
      ...
      private function loadConfigData() : void
      {
         ...
         for(var i:int = this._localW; i < w; i++)
         {
            for(j = 0; j < this._h; j++)
            {
               this._grid.setNodeWalkAble(i,j,this._configBytes.readByte() == 1 ? true : false);
