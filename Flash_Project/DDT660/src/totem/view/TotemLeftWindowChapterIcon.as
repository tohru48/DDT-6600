package totem.view
{
   ...
   public class TotemLeftWindowChapterIcon extends Sprite implements Disposeable
   {
      ...
      public function TotemLeftWindowChapterIcon()
      {
         ...
         {
            tmp = new Bitmap(ClassUtils.CreatInstance("asset.totem.chapterIcon" + i),"auto",true);
            this._iconList.push(tmp);
         }
      ...
      public function show(chapterIndex:int) : void
      {
         ...
         this._icon = this._iconList[chapterIndex - 1];
         this._iconSprite.addChild(this._icon);
         this._tipView.show(chapterIndex);
      }
      
      private function showTip(event:MouseEvent) : void
      {
         var tmpPoint:Point = null;
         tmpPoint = this.localToGlobal(new Point(this._iconSprite.width + 5,this._iconSprite.height / 2));
