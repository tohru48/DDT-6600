package times
{
   ...
   public class TimesController extends EventDispatcher
   {
      ...
      private var _eggLoc:Point = new Point(-1,-1);
      ...
      private function __gotoNextContent(param1:TimesEvent) : void
      {
         ...
         this.currentPointer = this.currentPointer < 3 ? this.currentPointer + 1 : 0;
         this._timesView.menuSelected(this.currentPointer);
         this.tryRemoveAllViews();
      ...
      {
         var _loc2_:* = undefined;
         this.currentPointer = this.currentPointer > 0 ? this.currentPointer - 1 : this._contentViews.length - 1;
         this._timesView.menuSelected(this.currentPointer);
         this.tryRemoveAllViews();
      ...
      private function __pushTipItems(param1:TimesEvent) : void
      {
         ...
         if(Boolean(this._tipItemMcs[_loc4_]))
         {
      ...
      private function loadThumbnail() : void
      {
         ...
         while(i < this._model.contentInfos.length)
         {
            ...
            while(j < this._model.contentInfos[i].length)
            {
               ...
               {
                  arr.push(LoadResourceManager.Instance.createLoader(this._model.webPath + info.thumbnailPath,BaseLoader.BITMAP_LOADER));
      ...
      public function dispose() : void
      {
         ...
         {
            if(Boolean(this._tipItemMcs[_loc3_]))
            {
               if(Boolean(this._tipItemMcs[_loc3_].parent))
               {
      ...
      public function set isShowUpdateView(param1:Boolean) : void
      {
         ...
         var _loc3_:Vector.<TimesPicInfo> = null;
         var _loc4_:TimesPicInfo = null;
         var _loc5_:TimesPicInfo = null;
         var _loc6_:Vector.<TimesPicInfo> = null;
         if(this._isShowUpdateView == param1)
         {
            return;
         }
         this._isShowUpdateView = param1;
         if(this._isShowUpdateView)
         {
            _loc2_ = [null];
            this._thumbnailLoaders[0] = _loc2_.concat(this._thumbnailLoaders[0] as Array);
            _loc3_ = this._model.contentInfos[0];
            for each(_loc4_ in _loc3_)
            {
               ++_loc4_.page;
            }
            _loc5_ = new TimesPicInfo();
            _loc5_.category = 0;
            _loc5_.page = 0;
            if(Boolean(_loc3_[0]))
            {
               _loc5_.type = _loc3_[0].type;
            }
            _loc6_ = new Vector.<TimesPicInfo>();
            _loc6_.push(_loc5_);
            this._model.contentInfos[0] = _loc6_.concat(this._model.contentInfos[0]);
         }
      }
   }
}
