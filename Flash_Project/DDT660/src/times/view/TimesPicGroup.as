package times.view
{
   ...
   public class TimesPicGroup extends Sprite implements Disposeable
   {
      ...
      {
         var _loc2_:TimesUpdateView = null;
         this._pics = new Vector.<Sprite>();
         this._timer = new Timer(5000);
         this._maxIdx = this._infos.length;
         var _loc1_:int = 0;
         while(_loc1_ < this._maxIdx)
         {
            if(this._index == 0 && TimesController.Instance.isShowUpdateView && _loc1_ == 0)
            {
               _loc2_ = new TimesUpdateView(TimesController.Instance.updateContenList,true);
               _loc2_.x = 15;
               _loc2_.y = 47;
               this._pics.push(_loc2_);
            }
            else
            {
               this._pics.push(new TimesPicBase(this._infos[_loc1_]));
               (this._pics[_loc1_] as TimesPicBase).load();
            }
            _loc1_++;
         }
