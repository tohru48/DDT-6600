package chickActivation.model
{
   ...
   public class ChickActivationModel extends EventDispatcher
   {
      ...
      public function findQualityValue(qualityKey:String) : int
      {
         ...
         if(this.qualityDic.hasOwnProperty(qualityKey))
         {
            qualityValue = int(this.qualityDic[qualityKey]);
         }
         return qualityValue;
      }
      ...
      {
         var temp:Number = 24 * 60 * 60 * 1000;
         var day:int = 0;
         if(Boolean(this.isKeyOpened) && Boolean(this.keyOpenedTime))
         {
            day = Math.ceil((60 * temp - (TimeManager.Instance.Now().time - this.keyOpenedTime.time)) / temp);
            if(day > 60)
            {
               day = 60;
            }
      ...
      {
         return (this.gainArr[11] & 1 << level - 1) > 0;
      }
