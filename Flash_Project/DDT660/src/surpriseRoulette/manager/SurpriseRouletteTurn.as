package surpriseRoulette.manager
{
   ...
   public class SurpriseRouletteTurn extends EventDispatcher
   {
      ...
      public function SurpriseRouletteTurn(target:IEventDispatcher = null)
      {
         super(target);
         this.init();
         this.initEvent();
      ...
      private function _clearPrevSelct(now:int) : void
      {
         if(this._turnType == TYPE_SPEED_UNCHANGE)
         {
            this._goodsList[this.getNum(now - 1)].setEffect(TWO);
            this._goodsList[this.getNum(now - 2)].setEffect(ONE);
            this._goodsList[this.getNum(now - 3)].setEffect(ZERO);
         }
         else
         {
            this._goodsList[this.getNum(now - 1)].setEffect(ZERO);
            this._goodsList[this.getNum(now - 2)].setEffect(ZERO);
            this._goodsList[this.getNum(now - 3)].setEffect(ZERO);
         }
      }
      ...
      private function getNum(num:int) : int
      {
         if(num < 0)
         {
            return num + this._goodsList.length;
         }
         return num;
      }
      ...
      public function turnPlate(list:Vector.<SurpriseRouletteCell>, _select:int) : void
      {
      ...
      public function selectedGoodsNumber(value:int) : void
      {
         this._moderationNumber = (this._delay[2] - this._delay[1]) / SPEEDDOWN_RATE;
         var m:int = value - this._moderationNumber;
         while(m < 0)
         {
