package im.chatFrame
{
   ...
   public class PrivateChatFrame extends MinimizeFrame
   {
      ...
      private function initView() : void
      {
         ...
         this._targetProtrait.buttonMode = true;
         ...
         this._targetProtrait.mouseEnabled = true;
         ...
         this._selfPortrait.info = self;
         this._selfLevelT.text = LanguageMgr.GetTranslation("IM.ChatFrame.level");
         this._selfLevel.setSize(LevelIcon.SIZE_SMALL);
         this._selfLevel.setInfo(self.Grade,self.Repute,self.WinCount,self.TotalCount,self.FightPower,self.Offer,false,true);
         ...
         if(self.IsVIP)
         {
            this._selfVipName = VipController.instance.getVipNameTxt(84,self.typeVIP);
            this._selfVipName.textSize = 14;
            this._selfVipName.x = this._selfName.x;
            this._selfVipName.y = this._selfName.y;
            this._selfVipName.text = self.NickName;
            addToContent(this._selfVipName);
         }
         else
         {
            this._selfName.text = self.NickName;
      ...
      public function addAllMessage(messages:Vector.<String>) : void
      {
         ...
         {
            this._output.htmlText += messages[i] + "<br/>";
         }
         this._output.textField.setSelection(this._output.text.length - 1,this._output.text.length - 1);
         this._output.upScrollArea();
      }
      ...
      private function initEvent() : void
      {
         ...
         this._targetProtrait.addEventListener(MouseEvent.CLICK,this.__targetProtraitClick);
      ...
      private function removeEvent() : void
      {
         ...
         this._targetProtrait.removeEventListener(MouseEvent.CLICK,this.__targetProtraitClick);
      ...
      protected function __targetProtraitClick(event:MouseEvent) : void
      {
         if(!this._nameTip)
         {
            this._nameTip = new ChatNamePanel();
         }
         this._nameTip.x = event.stageX;
         this._nameTip.y = event.stageY;
         this._nameTip.playerName = this._info.NickName;
      ...
      {
         if(event.responseCode == FrameEvent.CLOSE_CLICK)
         {
            SoundManager.instance.play("008");
            this._recordFrame.parent.removeChild(this._recordFrame);
            this._show = false;
      ...
      protected function __sendHandler(event:MouseEvent) : void
      {
         ...
         {
            str = this._input.text.replace(/</g,"&lt;").replace(/>/g,"&gt;");
            str = FilterWordManager.filterWrod(str);
            SocketManager.Instance.out.sendOneOnOneTalk(this._info.ID,str);
            this._input.text = "";
         }
         else
         {
