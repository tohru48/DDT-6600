package auctionHouse.view
{
   ...
   public class BidMoneyView extends Sprite implements Disposeable
   {
      ...
      internal function canMoneyBid(price:int) : void
      {
         ...
         this._canMoney = true;
         this._canGold = false;
         this._money.text = price.toString();
         this._money.setFocus();
         this._money.setSelection(price.toString().length,price.toString().length);
      }
      ...
      internal function getData() : Number
      {
         ...
         if(!this._canGold)
         {
            if(this._canMoney)
            {
               result = Number(this._money.text);
            }
         }
         return result;
      }
      ...
      {
         if(Number(this._money.text) + Number(event.text) > PlayerManager.Instance.Self.Money || Number(this._money.text) + Number(event.text) == 0)
         {
            return;
         }
      }
