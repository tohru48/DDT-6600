package farm.viewx
{
   ...
   public class FarmBuyExpFrame extends BaseAlerFrame
   {
      ...
      override protected function init() : void
      {
         ...
         var exp:int = int(ItemManager.Instance.getTemplateById(EquipType.SUPER_PET_EXP_FOOD).Property3);
         ...
         {
            _foodNumTextText = this._currentSuperPetFoodPriceInfo.ItemCount.toString();
         }
         this._text.htmlText = LanguageMgr.GetTranslation("farm.viewx.farmBuyExpFrame.explain",this._currentMoney.toString(),_foodNumTextText,exp);
      ...
      protected function __updateNum(event:Event) : void
      {
         ...
         if(FarmModelController.instance.model.buyExpRemainNum > 0)
         {
            ...
            exp = int(ItemManager.Instance.getTemplateById(EquipType.SUPER_PET_EXP_FOOD).Property3);
            this._text.htmlText = LanguageMgr.GetTranslation("farm.viewx.farmBuyExpFrame.explain",this._currentMoney.toString(),this._currentSuperPetFoodPriceInfo.ItemCount.toString(),exp);
         }
      }
      ...
      protected function __onFrameEvent(event:FrameEvent) : void
      {
         if(event.responseCode == FrameEvent.ENTER_CLICK || event.responseCode == FrameEvent.SUBMIT_CLICK)
         {
            ...
            this._currentMoney = FarmModelController.instance.getCurrentMoney();
            if(!(_isBand && PlayerManager.Instance.Self.BandMoney >= this._currentMoney))
            {
