package email.model
{
   ...
   public class EmailModel extends EventDispatcher
   {
      ...
      public function getNoReadMails() : void
      {
         ...
         {
            if(Boolean(SharedManager.Instance.spacialReadedMail[PlayerManager.Instance.Self.ID]) && SharedManager.Instance.spacialReadedMail[PlayerManager.Instance.Self.ID].indexOf(info.ID) > -1)
            {
               info.IsRead = true;
            }
            if(!info.IsRead)
      ...
      {
         var le:int = int(this._emails.length);
      ...
      public function getViewData() : Array
      {
         ...
         if(Boolean(this.currentDate))
         {
            ...
            end = begin + 7 > this.currentDate.length ? int(this.currentDate.length) : begin + 7;
