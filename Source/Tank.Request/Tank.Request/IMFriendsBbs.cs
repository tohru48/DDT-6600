...
namespace Tank.Request
{
	...
	public class IMFriendsBbs : IHttpHandler
	{
		...
		public class Normal : IAgentFriends
		{
			...
			public string FriendsString(string uid)
			{
				try
				{
					...
					string text = "";
					Url = string.Format(CultureInfo.InvariantCulture, FriendInterface, new object[1] { uid });
					string page = WebsResponse.GetPage(Url, "", "utf-8", out text);
					if (text == "")
					{
						return page;
					}
		...
		public void ProcessRequest(HttpContext context)
		{
			...
			IAgentFriends agentFriends = new Normal();
			StringBuilder stringBuilder = new StringBuilder();
			HttpContext current = HttpContext.Current;
			string text = agentFriends.FriendsString(current.Request.Params["Uid"]);
