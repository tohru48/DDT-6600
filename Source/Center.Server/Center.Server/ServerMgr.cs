...
namespace Center.Server
{
	...
	{
		private static readonly ILog zvbsIfkgLx;
		...
		public static bool Start()
		{
			...
			try
			{
				...
				ServerMgr.zvbsIfkgLx.Info("Load server list from db.");
				result = true;
			}
			catch (System.Exception ex)
			{
				ServerMgr.zvbsIfkgLx.ErrorFormat("Load server list from db failed:{0}", ex);
				result = false;
			}
			return result;
		}
		public static bool ReLoadServerList()
		{
			...
			try
			{
				...
				ServerMgr.zvbsIfkgLx.Info("ReLoad server list from db.");
				result = true;
			}
			catch (System.Exception ex)
			{
				ServerMgr.zvbsIfkgLx.ErrorFormat("ReLoad server list from db failed:{0}", ex);
				result = false;
			}
			return result;
		}
		...
		public static void SaveToDatabase()
		{
			try
			{
				using (ServiceBussiness serviceBussiness = new ServiceBussiness())
				{
					foreach (ServerInfo current in ServerMgr.dictionary_0.Values)
					{
			...
			{
				ServerMgr.zvbsIfkgLx.Error("Save server state", ex);
			}
		}
		public ServerMgr()
		{
		...
		static ServerMgr()
		{
			...
			ServerMgr.zvbsIfkgLx = LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);
