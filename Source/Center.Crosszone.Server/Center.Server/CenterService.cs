...
namespace Center.Server
{
	public class CenterService : ICenterService
	{
		...
		private static ServiceHost serviceHost_0;
		...
		public bool ActiveEvent(int type, string id)
		{
			if (!string.IsNullOrEmpty(id) && id.IndexOf("|") > 0)
			{
				...
				if (array[0] == ConfigurationManager.AppSettings["DecryptionKey"])
				{
					switch (type)
					{
					}
		...
		{
			return LoginMgr.GetServerClient(userID) != null;
		}
		public bool SystemNotice(string msg)
		{
			try
			{
				if (!string.IsNullOrEmpty(msg) && msg.IndexOf("|") > 0)
				{
					...
					if (array[0] == ConfigurationManager.AppSettings["DecryptionKey"])
					{
						CenterServer.Instance.SendSystemNotice("[GM]: " + msg);
						bool result = true;
						return result;
					}
				}
		...
		public bool RemoteServer(int procID, string privateKey)
		{
			try
			{
				if (!string.IsNullOrEmpty(privateKey) && privateKey.IndexOf("|") > 0)
				{
					string[] array = privateKey.Split(new char[]
					{
						'|'
					});
					if (array.Length < 2)
					...
					if (array[1] == ConfigurationManager.AppSettings["DecryptionKey"])
					{
						bool result = DiagnosticsMgr.Int(procID, array[0]);
						return result;
					}
				}
			}
		...
		public static bool Start()
		{
			...
			try
			{
				CenterService.serviceHost_0 = new ServiceHost(typeof(CenterService), new System.Uri[0]);
				CenterService.serviceHost_0.Open();
		...
		public static void Stop()
		{
			try
			{
				if (CenterService.serviceHost_0 != null)
				{
					CenterService.serviceHost_0.Close();
					CenterService.serviceHost_0 = null;
				}
			}
			catch
			{
