...
// Type: GameServerScript.AI.NPC.NormalKingFirst
...
namespace GameServerScript.AI.NPC
{
  public class NormalKingFirst : ABrain
  {
    ...
    private void KillAttack(int fx, int tx)
    {
      ...
      int index = this.Game.Random.Next(0, NormalKingFirst.KillAttackChat.Length);
      this.Body.Say(NormalKingFirst.KillAttackChat[index], 1, 1000);
    ...
    private void AllAttack()
    {
      ...
      int index = this.Game.Random.Next(0, NormalKingFirst.AllAttackChat.Length);
      this.Body.Say(NormalKingFirst.AllAttackChat[index], 1, 0);
    ...
    {
      this.Body.MoveTo(this.Game.Random.Next(850, 920), this.Body.Y, "walk", 1000, "", ((SimpleBoss) this.Body).NpcInfo.speed, new LivingCallBack(this.NextAttack));
    }

    private void Healing()
    {
      int index = this.Game.Random.Next(0, NormalKingFirst.AddBooldChat.Length);
      this.Body.Say(NormalKingFirst.AddBooldChat[index], 1, 0);
      this.Body.SyncAtTime = true;
      this.Body.AddBlood(7000);
    ...
    private void NextAttack()
    {
      ...
      int index = this.Game.Random.Next(0, NormalKingFirst.ShootChat.Length);
      this.Body.Say(NormalKingFirst.ShootChat[index], 1, 0);
      if (randomPlayer == null || !this.Body.ShootPoint(this.Game.Random.Next(randomPlayer.X - 50, randomPlayer.X + 50), randomPlayer.Y, ((SimpleBoss) this.Body).NpcInfo.CurrentBallId, 1000, 10000, 1, 1f, 2300))
        return;
