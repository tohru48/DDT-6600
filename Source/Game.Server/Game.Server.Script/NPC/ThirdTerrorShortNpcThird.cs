...
// Type: GameServerScript.AI.NPC.ThirdTerrorShortNpcThird
  ...
  public class ThirdTerrorShortNpcThird : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(ThirdTerrorShortNpcThird.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = ThirdTerrorShortNpcThird.random.Next(0, ThirdTerrorShortNpcThird.listChat.Length);
      return ThirdTerrorShortNpcThird.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdTerrorShortNpcThird.random.Next(1, 4) : ThirdTerrorShortNpcThird.random.Next(1, 3)) : ThirdTerrorShortNpcThird.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdTerrorShortNpcThird.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdTerrorShortNpcThird.random.Next(0, 5000);
