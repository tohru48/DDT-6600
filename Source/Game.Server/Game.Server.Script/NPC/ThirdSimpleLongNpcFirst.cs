...
// Type: GameServerScript.AI.NPC.ThirdSimpleLongNpcFirst
...
namespace GameServerScript.AI.NPC
{
  public class ThirdSimpleLongNpcFirst : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, ThirdSimpleLongNpcFirst.KillAttackChat.Length);
      this.Body.Say(ThirdSimpleLongNpcFirst.KillAttackChat[index], 0, 1000);
      this.Body.CurrentDamagePlus = 1f;
    ...
    public override void OnKillPlayerSay()
    {
      ...
      int index = this.Game.Random.Next(0, ThirdSimpleLongNpcFirst.KillPlayerChat.Length);
      this.Body.Say(ThirdSimpleLongNpcFirst.KillPlayerChat[index], 1, 0, 2000);
    }
    ...
    {
      int index1 = this.Game.Random.Next(0, ThirdSimpleLongNpcFirst.ShootedChat.Length);
      if (this.isSay == 0 && this.Body.IsLiving)
      {
        this.Body.Say(ThirdSimpleLongNpcFirst.ShootedChat[index1], 1, 900, 0);
        this.isSay = 1;
      }
      if (this.Body.IsLiving)
        return;
      int index2 = this.Game.Random.Next(0, ThirdSimpleLongNpcFirst.DiedChat.Length);
      this.Body.Say(ThirdSimpleLongNpcFirst.DiedChat[index2], 1, 100, 2000);
    }

    public static string GetOneChat()
    {
      int index = ThirdSimpleLongNpcFirst.random.Next(0, ThirdSimpleLongNpcFirst.listChat.Length);
      return ThirdSimpleLongNpcFirst.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdSimpleLongNpcFirst.random.Next(1, 4) : ThirdSimpleLongNpcFirst.random.Next(1, 3)) : ThirdSimpleLongNpcFirst.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdSimpleLongNpcFirst.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdSimpleLongNpcFirst.random.Next(0, 5000);
