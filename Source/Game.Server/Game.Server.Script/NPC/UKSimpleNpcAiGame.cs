...
// Type: GameServerScript.AI.NPC.UKSimpleNpcAiGame
...
{
  public class UKSimpleNpcAiGame : ABrain
  {
    protected Player m_targer;
    private static Random random = new Random();
    private static string[] listChat = new string[1]
    {
      "Ajudarei a acabar a guerra,<br>e saíremos vitoriosos"
    };
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(UKSimpleNpcAiGame.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = UKSimpleNpcAiGame.random.Next(0, UKSimpleNpcAiGame.listChat.Length);
      return UKSimpleNpcAiGame.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? UKSimpleNpcAiGame.random.Next(1, 4) : UKSimpleNpcAiGame.random.Next(1, 3)) : UKSimpleNpcAiGame.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = UKSimpleNpcAiGame.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = UKSimpleNpcAiGame.random.Next(0, 5000);
          livings[index].Say(UKSimpleNpcAiGame.GetOneChat(), 0, delay);
          ++num;
        }
      }
    }
