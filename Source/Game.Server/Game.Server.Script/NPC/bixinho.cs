...
// Type: GameServerScript.AI.NPC.bixinho
...
{
  public class bixinho : ABrain
  {
    protected Player m_targer;
    private static Random random = new Random();
    private static string[] listChat = new string[4]
    {
      "Cheguei nessa porraaaaaaaaa o/o/",
      "hahahahaha",
      "Vai Morrer !!",
      "Eae doido, firmeza?"
    };
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(bixinho.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = bixinho.random.Next(0, bixinho.listChat.Length);
      return bixinho.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? bixinho.random.Next(1, 4) : bixinho.random.Next(1, 3)) : bixinho.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = bixinho.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = bixinho.random.Next(0, 5000);
