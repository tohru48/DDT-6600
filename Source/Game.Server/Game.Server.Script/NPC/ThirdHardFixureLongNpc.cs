...
// Type: GameServerScript.AI.NPC.ThirdHardFixureLongNpc
...
namespace GameServerScript.AI.NPC
{
  public class ThirdHardFixureLongNpc : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, ThirdHardFixureLongNpc.KillAttackChat.Length);
      this.Body.Say(ThirdHardFixureLongNpc.KillAttackChat[index], 1, 1000);
    ...
    public override void OnKillPlayerSay()
    {
      ...
      int index = this.Game.Random.Next(0, ThirdHardFixureLongNpc.KillPlayerChat.Length);
      this.Body.Say(ThirdHardFixureLongNpc.KillPlayerChat[index], 1, 0, 2000);
    }
    ...
    {
      int index1 = this.Game.Random.Next(0, ThirdHardFixureLongNpc.ShootedChat.Length);
      if (this.isSay == 0 && this.Body.IsLiving)
      {
        this.Body.Say(ThirdHardFixureLongNpc.ShootedChat[index1], 1, 900, 0);
        this.isSay = 1;
      }
      if (this.Body.IsLiving)
        return;
      int index2 = this.Game.Random.Next(0, ThirdHardFixureLongNpc.DiedChat.Length);
      this.Body.Say(ThirdHardFixureLongNpc.DiedChat[index2], 1, 100, 2000);
    }

    public static string GetOneChat()
    {
      int index = ThirdHardFixureLongNpc.random.Next(0, ThirdHardFixureLongNpc.listChat.Length);
      return ThirdHardFixureLongNpc.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdHardFixureLongNpc.random.Next(1, 4) : ThirdHardFixureLongNpc.random.Next(1, 3)) : ThirdHardFixureLongNpc.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdHardFixureLongNpc.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdHardFixureLongNpc.random.Next(0, 5000);
