...
// Type: GameServerScript.AI.NPC.ThirdTerrorFixureLongNpc
...
namespace GameServerScript.AI.NPC
{
  public class ThirdTerrorFixureLongNpc : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, ThirdTerrorFixureLongNpc.KillAttackChat.Length);
      this.Body.Say(ThirdTerrorFixureLongNpc.KillAttackChat[index], 1, 1000);
    ...
    public override void OnKillPlayerSay()
    {
      ...
      int index = this.Game.Random.Next(0, ThirdTerrorFixureLongNpc.KillPlayerChat.Length);
      this.Body.Say(ThirdTerrorFixureLongNpc.KillPlayerChat[index], 1, 0, 2000);
    }
    ...
    {
      int index1 = this.Game.Random.Next(0, ThirdTerrorFixureLongNpc.ShootedChat.Length);
      if (this.isSay == 0 && this.Body.IsLiving)
      {
        this.Body.Say(ThirdTerrorFixureLongNpc.ShootedChat[index1], 1, 900, 0);
        this.isSay = 1;
      }
      if (this.Body.IsLiving)
        return;
      int index2 = this.Game.Random.Next(0, ThirdTerrorFixureLongNpc.DiedChat.Length);
      this.Body.Say(ThirdTerrorFixureLongNpc.DiedChat[index2], 1, 100, 2000);
    }

    public static string GetOneChat()
    {
      int index = ThirdTerrorFixureLongNpc.random.Next(0, ThirdTerrorFixureLongNpc.listChat.Length);
      return ThirdTerrorFixureLongNpc.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdTerrorFixureLongNpc.random.Next(1, 4) : ThirdTerrorFixureLongNpc.random.Next(1, 3)) : ThirdTerrorFixureLongNpc.random.Next(0, 2);
      if (length <= 0)
        return;
