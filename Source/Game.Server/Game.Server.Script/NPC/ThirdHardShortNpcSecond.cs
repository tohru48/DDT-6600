...
// Type: GameServerScript.AI.NPC.ThirdHardShortNpcSecond
  ...
  public class ThirdHardShortNpcSecond : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(ThirdHardShortNpcSecond.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = ThirdHardShortNpcSecond.random.Next(0, ThirdHardShortNpcSecond.listChat.Length);
      return ThirdHardShortNpcSecond.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdHardShortNpcSecond.random.Next(1, 4) : ThirdHardShortNpcSecond.random.Next(1, 3)) : ThirdHardShortNpcSecond.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdHardShortNpcSecond.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdHardShortNpcSecond.random.Next(0, 5000);
