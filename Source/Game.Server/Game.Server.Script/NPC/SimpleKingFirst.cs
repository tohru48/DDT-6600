...
// Type: GameServerScript.AI.NPC.SimpleKingFirst
...
namespace GameServerScript.AI.NPC
{
  public class SimpleKingFirst : ABrain
  {
    ...
    private void KillAttack(int fx, int tx)
    {
      ...
      int index = this.Game.Random.Next(0, SimpleKingFirst.KillAttackChat.Length);
      this.Body.Say(SimpleKingFirst.KillAttackChat[index], 1, 1000);
    ...
    private void AllAttack()
    {
      ...
      int index = this.Game.Random.Next(0, SimpleKingFirst.AllAttackChat.Length);
      this.Body.Say(SimpleKingFirst.AllAttackChat[index], 1, 0);
    ...
    {
      this.Body.MoveTo(this.Game.Random.Next(800, 980), this.Body.Y, "walk", 1000, "", ((SimpleBoss) this.Body).NpcInfo.speed, new LivingCallBack(this.NextAttack));
    }

    private void Healing()
    {
      int index = this.Game.Random.Next(0, SimpleKingFirst.AddBooldChat.Length);
      this.Body.Say(SimpleKingFirst.AddBooldChat[index], 1, 0);
    ...
    private void NextAttack()
    {
      ...
      int index = this.Game.Random.Next(0, SimpleKingFirst.ShootChat.Length);
      this.Body.Say(SimpleKingFirst.ShootChat[index], 1, 0);
      if (randomPlayer == null)
        return;
      int x = this.Game.Random.Next(randomPlayer.X - 50, randomPlayer.X + 50);
      if (this.Body.ShootPoint(x, randomPlayer.Y, ((SimpleBoss) this.Body).NpcInfo.CurrentBallId, 1000, 10000, 1, 1f, 2300))
        this.Body.PlayMovie("beat2", 1500, 0);
      if (this.Body.ShootPoint(x, randomPlayer.Y, ((SimpleBoss) this.Body).NpcInfo.CurrentBallId, 1000, 10000, 1, 1f, 4100))
        this.Body.PlayMovie("beat2", 3300, 0);
    }
