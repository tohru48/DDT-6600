...
// Type: GameServerScript.AI.NPC.ThirdSimpleShortNpcThird
  ...
  public class ThirdSimpleShortNpcThird : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(ThirdSimpleShortNpcThird.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = ThirdSimpleShortNpcThird.random.Next(0, ThirdSimpleShortNpcThird.listChat.Length);
      return ThirdSimpleShortNpcThird.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdSimpleShortNpcThird.random.Next(1, 4) : ThirdSimpleShortNpcThird.random.Next(1, 3)) : ThirdSimpleShortNpcThird.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdSimpleShortNpcThird.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdSimpleShortNpcThird.random.Next(0, 5000);
