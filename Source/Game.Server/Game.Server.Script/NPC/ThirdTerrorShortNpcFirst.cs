...
// Type: GameServerScript.AI.NPC.ThirdTerrorShortNpcFirst
  ...
  public class ThirdTerrorShortNpcFirst : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(ThirdTerrorShortNpcFirst.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = ThirdTerrorShortNpcFirst.random.Next(0, ThirdTerrorShortNpcFirst.listChat.Length);
      return ThirdTerrorShortNpcFirst.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdTerrorShortNpcFirst.random.Next(1, 4) : ThirdTerrorShortNpcFirst.random.Next(1, 3)) : ThirdTerrorShortNpcFirst.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdTerrorShortNpcFirst.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdTerrorShortNpcFirst.random.Next(0, 5000);
