...
// Type: GameServerScript.AI.NPC.ThirdNormalShortNpcThird
  ...
  public class ThirdNormalShortNpcThird : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(ThirdNormalShortNpcThird.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = ThirdNormalShortNpcThird.random.Next(0, ThirdNormalShortNpcThird.listChat.Length);
      return ThirdNormalShortNpcThird.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdNormalShortNpcThird.random.Next(1, 4) : ThirdNormalShortNpcThird.random.Next(1, 3)) : ThirdNormalShortNpcThird.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdNormalShortNpcThird.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdNormalShortNpcThird.random.Next(0, 5000);
