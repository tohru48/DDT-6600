...
// Type: GameServerScript.AI.NPC.FiveHardThirdNpc
  ...
  public class FiveHardThirdNpc : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(FiveHardThirdNpc.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = FiveHardThirdNpc.random.Next(0, FiveHardThirdNpc.listChat.Length);
      return FiveHardThirdNpc.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? FiveHardThirdNpc.random.Next(1, 4) : FiveHardThirdNpc.random.Next(1, 3)) : FiveHardThirdNpc.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        ...
        if (!livings[index].IsSay)
        {
          ...
          FiveHardThirdNpc.random.Next(0, 5000);
          ++num;
        }
      }
    }
