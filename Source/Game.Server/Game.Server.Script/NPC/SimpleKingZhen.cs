...
// Type: GameServerScript.AI.NPC.SimpleKingZhen
...
namespace GameServerScript.AI.NPC
{
  public class SimpleKingZhen : ABrain
  {
    ...
    public void HalfAttack()
    {
      ...
      int index = this.Game.Random.Next(0, SimpleKingZhen.SealChat.Length);
      this.Body.Say(SimpleKingZhen.AllAttackChat[index], 1, 500);
    ...
    public void Summon()
    {
      int index = this.Game.Random.Next(0, SimpleKingZhen.CallChat.Length);
      this.Body.Say(SimpleKingZhen.CallChat[index], 1, 0);
    ...
    {
      int index = this.Game.Random.Next(0, SimpleKingZhen.SealChat.Length);
      this.Body.Say(SimpleKingZhen.SealChat[index], 1, 0);
    ...
    public void Angger()
    {
      int index = this.Game.Random.Next(0, SimpleKingZhen.AngryChat.Length);
      this.Body.Say(SimpleKingZhen.AngryChat[index], 1, 0);
      this.Body.State = 1;
      this.Dander += 100;
    ...
    public void KillAttack(int fx, int tx)
    {
      ...
      int index = this.Game.Random.Next(0, SimpleKingZhen.KillAttackChat.Length);
      this.Body.Say(SimpleKingZhen.KillAttackChat[index], 1, 500);
      this.Body.PlayMovie("beatB", 2500, 0);
