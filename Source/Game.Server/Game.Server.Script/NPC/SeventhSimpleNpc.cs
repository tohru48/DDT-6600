...
// Type: GameServerScript.AI.NPC.SeventhSimpleNpc
  ...
  public class SeventhSimpleNpc : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(SeventhSimpleNpc.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = SeventhSimpleNpc.random.Next(0, SeventhSimpleNpc.listChat.Length);
      return SeventhSimpleNpc.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? SeventhSimpleNpc.random.Next(1, 4) : SeventhSimpleNpc.random.Next(1, 3)) : SeventhSimpleNpc.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = SeventhSimpleNpc.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = SeventhSimpleNpc.random.Next(0, 5000);
