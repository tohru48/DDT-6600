...
// Type: GameServerScript.AI.NPC.BrazucaNPC
...
{
  public class BrazucaNPC : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(BrazucaNPC.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = BrazucaNPC.random.Next(0, BrazucaNPC.listChat.Length);
      return BrazucaNPC.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? BrazucaNPC.random.Next(1, 4) : BrazucaNPC.random.Next(1, 3)) : BrazucaNPC.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = BrazucaNPC.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = BrazucaNPC.random.Next(0, 5000);
