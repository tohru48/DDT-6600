...
// Type: GameServerScript.AI.NPC.FourNormalFrantCattleBoss
...
namespace GameServerScript.AI.NPC
{
  public class FourNormalFrantCattleBoss : ABrain
  {
    ...
    {
      if (this.Body.X > this.Game.Map.Info.ForegroundWidth / 2)
        this.Body.MoveTo(1, this.Body.Y, "walk", 1000, "", 24, new LivingCallBack(this.FallTo));
      if (this.Body.X < this.Game.Map.Info.ForegroundWidth / 2)
        this.Body.MoveTo(1600, this.Body.Y, "walk", 1000, "", 24, new LivingCallBack(this.FallTo));
      this.Body.RangeAttacking(this.Body.X - 2000, this.Body.X + 2000, "cry", 1200, (List<Player>) null);
    }
    ...
    private void KillAttack(int fx, int tx)
    {
      ...
      this.Body.Direction = this.Game.FindlivingbyDir(this.Body);
      int index = this.Game.Random.Next(0, FourNormalFrantCattleBoss.KillAttackChat.Length);
      this.Body.Say(FourNormalFrantCattleBoss.KillAttackChat[index], 1, 1000);
    ...
    public void FallTo()
    {
      if (this.Body.X > 700)
      {
        ...
        this.Body.FallFrom(1599, 900, "fallB", 10, 10, 10);
        this.Body.PlayMovie("fallB", 20, 0);
      }
      else
      {
        this.Body.ChangeDirection(1, 0);
        this.Body.FallFrom(1, 900, "fallB", 10, 10, 10);
        this.Body.PlayMovie("fallB", 20, 0);
      }
    }
  }
