...
// Type: GameServerScript.AI.NPC.ThirdHardShortNpcFirst
  ...
  public class ThirdHardShortNpcFirst : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(ThirdHardShortNpcFirst.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = ThirdHardShortNpcFirst.random.Next(0, ThirdHardShortNpcFirst.listChat.Length);
      return ThirdHardShortNpcFirst.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdHardShortNpcFirst.random.Next(1, 4) : ThirdHardShortNpcFirst.random.Next(1, 3)) : ThirdHardShortNpcFirst.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdHardShortNpcFirst.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdHardShortNpcFirst.random.Next(0, 5000);
