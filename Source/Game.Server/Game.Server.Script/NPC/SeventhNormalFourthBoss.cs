...
// Type: GameServerScript.AI.NPC.SeventhNormalFourthBoss
...
namespace GameServerScript.AI.NPC
{
  public class SeventhNormalFourthBoss : ABrain
  {
    ...
    {
      LanguageMgr.GetTranslation("Eu vou Te MATAR NUBB... HAHAHAHA")
    };
    private static string[] ShootChat = new string[1]
    {
      LanguageMgr.GetTranslation("Melhor DDTank é Aqui !")
    };
    ...
    {
      LanguageMgr.GetTranslation("Você não Resistira !")
    };
    ...
    {
      this.Body.MoveTo(this.Game.Random.Next(550, 800), this.Body.Y, "walk", 1000, "", 3, new LivingCallBack(this.GoAttack));
    }
    ...
    private void GoAttack()
    {
      this.Body.CurrentDamagePlus = 1f;
      this.Body.Direction = this.Game.FindlivingbyDir(this.Body);
      this.Body.PlayMovie("beatB", 5000, 2000);
      this.Body.RangeAttacking(this.Body.X - 5000, this.Body.X + 5000, "cry", 7000, (List<Player>) null);
      this.Body.SetRelateDemagemRect(-21, -87, 72, 59);
