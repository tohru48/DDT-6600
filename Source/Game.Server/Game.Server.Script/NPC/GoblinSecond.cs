...
// Type: GameServerScript.AI.NPC.GoblinSecond
...
namespace GameServerScript.AI.NPC
{
  public class GoblinSecond : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, GoblinSecond.KillAttackChat.Length);
      if (this.m_turn == 0)
      {
        this.Body.CurrentDamagePlus = 10f;
        this.Body.Say(GoblinSecond.KillAttackChat[index], 1, 13000);
      ...
      {
        this.Body.CurrentDamagePlus = 10f;
        this.Body.Say(GoblinSecond.KillAttackChat[index], 1, 0);
    ...
    {
      this.Body.PlayMovie("beatA", 1000, 10000);
      Player randomPlayer = this.Game.FindRandomPlayer();
      if (randomPlayer == null || randomPlayer.IsFrost || !this.Body.ShootPoint(randomPlayer.X, randomPlayer.Y, 56, 1000, 10000, 1, 2.5f, 2000))
        ;
    }
