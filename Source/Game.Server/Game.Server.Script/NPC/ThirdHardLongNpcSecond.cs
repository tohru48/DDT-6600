...
// Type: GameServerScript.AI.NPC.ThirdHardLongNpcSecond
...
namespace GameServerScript.AI.NPC
{
  public class ThirdHardLongNpcSecond : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, ThirdHardLongNpcSecond.KillAttackChat.Length);
      this.Body.Say(ThirdHardLongNpcSecond.KillAttackChat[index], 1, 1000);
    ...
    public override void OnKillPlayerSay()
    {
      ...
      int index = this.Game.Random.Next(0, ThirdHardLongNpcSecond.KillPlayerChat.Length);
      this.Body.Say(ThirdHardLongNpcSecond.KillPlayerChat[index], 1, 0, 2000);
    }
    ...
    {
      int index1 = this.Game.Random.Next(0, ThirdHardLongNpcSecond.ShootedChat.Length);
      if (this.isSay == 0 && this.Body.IsLiving)
      {
        this.Body.Say(ThirdHardLongNpcSecond.ShootedChat[index1], 1, 900, 0);
        this.isSay = 1;
      }
      if (this.Body.IsLiving)
        return;
      int index2 = this.Game.Random.Next(0, ThirdHardLongNpcSecond.DiedChat.Length);
      this.Body.Say(ThirdHardLongNpcSecond.DiedChat[index2], 1, 100, 2000);
    }

    public static string GetOneChat()
    {
      int index = ThirdHardLongNpcSecond.random.Next(0, ThirdHardLongNpcSecond.listChat.Length);
      return ThirdHardLongNpcSecond.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdHardLongNpcSecond.random.Next(1, 4) : ThirdHardLongNpcSecond.random.Next(1, 3)) : ThirdHardLongNpcSecond.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdHardLongNpcSecond.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdHardLongNpcSecond.random.Next(0, 5000);
