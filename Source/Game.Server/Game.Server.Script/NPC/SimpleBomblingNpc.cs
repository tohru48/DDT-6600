...
// Type: GameServerScript.AI.NPC.SimpleBomblingNpc
...
namespace GameServerScript.AI.NPC
{
  public class SimpleBomblingNpc : ABrain
  {
    ...
    public void MoveToPlayer(Player player)
    {
      ...
      {
        if (this.Body.X + num >= player.X)
          this.Body.MoveTo(player.X - 10, this.Body.Y, "walk", 2000, "", ((SimpleNpc) this.Body).NpcInfo.speed, new LivingCallBack(this.Beat));
        else
          this.Body.MoveTo(this.Body.X + num, this.Body.Y, "walk", 2000, "", ((SimpleNpc) this.Body).NpcInfo.speed, new LivingCallBack(this.Beat));
      }
      else if (this.Body.X - num <= player.X)
        this.Body.MoveTo(player.X + 10, this.Body.Y, "walk", 2000, "", ((SimpleNpc) this.Body).NpcInfo.speed, new LivingCallBack(this.Beat));
      else
        this.Body.MoveTo(this.Body.X - num, this.Body.Y, "walk", 2000, "", ((SimpleNpc) this.Body).NpcInfo.speed, new LivingCallBack(this.Beat));
    }
