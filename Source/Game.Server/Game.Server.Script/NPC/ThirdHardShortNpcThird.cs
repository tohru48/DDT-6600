...
// Type: GameServerScript.AI.NPC.ThirdHardShortNpcThird
  ...
  public class ThirdHardShortNpcThird : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(ThirdHardShortNpcThird.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = ThirdHardShortNpcThird.random.Next(0, ThirdHardShortNpcThird.listChat.Length);
      return ThirdHardShortNpcThird.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdHardShortNpcThird.random.Next(1, 4) : ThirdHardShortNpcThird.random.Next(1, 3)) : ThirdHardShortNpcThird.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdHardShortNpcThird.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdHardShortNpcThird.random.Next(0, 5000);
