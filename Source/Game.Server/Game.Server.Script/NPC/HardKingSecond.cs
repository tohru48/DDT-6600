...
// Type: GameServerScript.AI.NPC.HardKingSecond
...
namespace GameServerScript.AI.NPC
{
  public class HardKingSecond : ABrain
  {
    ...
    {
      this.Body.MoveTo(this.Game.Random.Next(840, 910), this.Body.Y, "walk", 0, "", ((SimpleBoss) this.Body).NpcInfo.speed, new LivingCallBack(this.NextAttack));
    }
    ...
    private void AllAttack()
    {
      ...
      if (this.m_turn == 0)
      {
        int index = this.Game.Random.Next(0, HardKingSecond.AllAttackChat.Length);
        this.Body.Say(HardKingSecond.AllAttackChat[index], 1, 13000);
      ...
      {
        int index = this.Game.Random.Next(0, HardKingSecond.AllAttackChat.Length);
        this.Body.Say(HardKingSecond.AllAttackChat[index], 1, 0);
    ...
    {
      int index = this.Game.Random.Next(0, HardKingSecond.KillAttackChat.Length);
      if (this.m_turn == 0)
      {
        this.Body.CurrentDamagePlus = 10f;
        this.Body.Say(HardKingSecond.KillAttackChat[index], 1, 13000);
        this.Body.PlayMovie("beat1", 15000, 0);
      ...
      {
        this.Body.CurrentDamagePlus = 10f;
        this.Body.Say(HardKingSecond.KillAttackChat[index], 1, 0);
    ...
    private void ProtectingWall()
    {
      ...
      int index = this.Game.Random.Next(0, HardKingSecond.WallChat.Length);
      this.Body.Say(HardKingSecond.WallChat[index], 1, 0);
    }
    ...
    private void NextAttack()
    {
      ...
      for (int index1 = 0; index1 < num; ++index1)
      {
        ...
        int index2 = this.Game.Random.Next(0, HardKingSecond.ShootChat.Length);
        this.Body.Say(HardKingSecond.ShootChat[index2], 1, 0);
        if (randomPlayer.X > this.Body.X)
          this.Body.ChangeDirection(1, 500);
        else
          this.Body.ChangeDirection(-1, 500);
        if (randomPlayer != null && !randomPlayer.IsFrost && this.Body.ShootPoint(randomPlayer.X, randomPlayer.Y, 1, 1000, 10000, 1, 1.5f, 2000))
