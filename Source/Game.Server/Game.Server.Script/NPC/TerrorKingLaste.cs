...
// Type: GameServerScript.AI.NPC.TerrorKingLaste
...
{
  public class TerrorKingLaste : ABrain
  {
    private int attackingTurn = 1;
    private int Dander = 0;
    private int npcID = 110011;
    ...
    public void HalfAttack()
    {
      ...
      int index = this.Game.Random.Next(0, TerrorKingLaste.SealChat.Length);
      this.Body.Say(TerrorKingLaste.AllAttackChat[index], 1, 500);
    ...
    public void Summon()
    {
      int index = this.Game.Random.Next(0, TerrorKingLaste.CallChat.Length);
      this.Body.Say(TerrorKingLaste.CallChat[index], 1, 0);
    ...
    {
      int index = this.Game.Random.Next(0, TerrorKingLaste.SealChat.Length);
      this.Body.Say(TerrorKingLaste.SealChat[index], 1, 0);
    ...
    public void Angger()
    {
      int index = this.Game.Random.Next(0, TerrorKingLaste.AngryChat.Length);
      this.Body.Say(TerrorKingLaste.AngryChat[index], 1, 0);
      this.Body.State = 1;
      this.Dander += 100;
      this.Body.PlayMovie("angry", 1000, 0);
    ...
    public void GoOnAngger()
    {
      ...
      else
      {
        ...
        foreach (Player allLivingPlayer in this.Game.GetAllLivingPlayers())
          allLivingPlayer.AddEffect((AbstractEffect) new ContinueReduceBloodEffect(2, 150, (Living) allLivingPlayer), 0);
      }
    }
    ...
    public void KillAttack(int fx, int mx)
    {
      ...
      int index = this.Game.Random.Next(0, TerrorKingLaste.KillAttackChat.Length);
      this.Body.Say(TerrorKingLaste.KillAttackChat[index], 1, 500);
      this.Body.PlayMovie("beatB", 2500, 0);
    ...
    {
      ((SimpleBoss) this.Body).CreateChild(this.npcID, 800, 665, 180, 6, -1);
    }
  }
}
