...
// Type: GameServerScript.AI.NPC.SeventhHardSecondBoss
...
namespace GameServerScript.AI.NPC
{
  public class SeventhHardSecondBoss : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, SeventhHardSecondBoss.KillAttackChat.Length);
      this.Body.Say(SeventhHardSecondBoss.KillAttackChat[index], 1, 0);
    ...
    {
      Player randomPlayer = this.Game.FindRandomPlayer();
      if (randomPlayer == null || !this.Body.ShootPoint(randomPlayer.X, randomPlayer.Y, 83, 1000, 10000, 3, 1f, 3300))
        return;
    ...
    private void GoMovie()
    {
      ...
      this.Body.RangeAttacking(x - 200, x + 200, "cry", 3000, (List<Player>) null);
      this.moive = (PhysicalObj) ((PVEGame) this.Game).Createlayer(x, randomPlayer.Y, "moive", "asset.game.seven.choud", "out", 1, 0);
      this.Body.CallFuction(new LivingCallBack(this.GoAttacking), 3000);
    }
      ...
      if (!this.IsEixt && randomPlayer != null)
      {
        ...
        foreach (Player allLivingPlayer in this.Game.GetAllLivingPlayers())
        {
          ...
          if (allLivingPlayer.X > randomPlayer.X - num && allLivingPlayer.X < randomPlayer.X + num)
            allLivingPlayer.AddEffect((AbstractEffect) new ContinueReduceBlood(2, 900, (Living) allLivingPlayer), 0);
        }
      }
