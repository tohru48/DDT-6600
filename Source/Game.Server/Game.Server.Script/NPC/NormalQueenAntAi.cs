...
// Type: GameServerScript.AI.NPC.NormalQueenAntAi
...
namespace GameServerScript.AI.NPC
{
  public class NormalQueenAntAi : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, NormalQueenAntAi.KillAttackChat.Length);
      this.Body.Say(NormalQueenAntAi.KillAttackChat[index], 1, 1000);
    ...
    private void PersonalAttack()
    {
      ...
      int index = this.Game.Random.Next(0, NormalQueenAntAi.ShootChat.Length);
      this.Body.Say(NormalQueenAntAi.ShootChat[index], 1, 0);
    ...
    private void Summon()
    {
      int index = this.Game.Random.Next(0, NormalQueenAntAi.CallChat.Length);
      this.Body.Say(NormalQueenAntAi.CallChat[index], 1, 600);
      this.Body.PlayMovie("call", 1700, 2000);
    ...
    {
      int index1 = this.Game.Random.Next(0, NormalQueenAntAi.ShootedChat.Length);
      if (this.isSay == 0 && this.Body.IsLiving)
      {
        this.Body.Say(NormalQueenAntAi.ShootedChat[index1], 1, delay, 0);
        this.isSay = 1;
      }
      if (this.Body.IsLiving)
        return;
      int index2 = this.Game.Random.Next(0, NormalQueenAntAi.DiedChat.Length);
      this.Body.Say(NormalQueenAntAi.DiedChat[index2], 1, delay - 800, 2000);
    }
  }
}
