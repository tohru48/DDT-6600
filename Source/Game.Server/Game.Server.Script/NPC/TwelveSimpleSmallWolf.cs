...
// Type: GameServerScript.AI.NPC.TwelveSimpleSmallWolf
    ...
    public void MoveToPlayer(Player player)
    {
      ...
      if (player.Y < 420 && player.X < 210)
      {
        if (this.Body.Y > 420)
        {
          if (this.Body.X - num3 < 50)
            this.Body.MoveTo(25, this.Body.Y, "walk", 1200, "", 3, new LivingCallBack(this.MoveBeat));
          else
      ...
      else if (this.Body.Y < 420)
      {
        if (this.Body.X + num3 > 200)
          this.Body.MoveTo(200, this.Body.Y, "walk", 1200, "", 3, new LivingCallBack(this.MoveBeat));
      }
      else if (player.X > this.Body.X)
        this.Body.MoveTo(this.Body.X + num3, this.Body.Y, "walk", 1200, "", 3, new LivingCallBack(this.MoveBeat));
      else
