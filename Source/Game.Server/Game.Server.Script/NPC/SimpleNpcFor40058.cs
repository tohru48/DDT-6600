...
// Type: GameServerScript.AI.NPC.SimpleNpcFor40058
  ...
  public class SimpleNpcFor40058 : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(SimpleNpcFor40058.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = SimpleNpcFor40058.random.Next(0, SimpleNpcFor40058.listChat.Length);
      return SimpleNpcFor40058.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? SimpleNpcFor40058.random.Next(1, 4) : SimpleNpcFor40058.random.Next(1, 3)) : SimpleNpcFor40058.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = SimpleNpcFor40058.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = SimpleNpcFor40058.random.Next(0, 5000);
