...
// Type: GameServerScript.AI.NPC.ThirdSimpleFixureLongNpc
...
namespace GameServerScript.AI.NPC
{
  public class ThirdSimpleFixureLongNpc : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, ThirdSimpleFixureLongNpc.KillAttackChat.Length);
      this.Body.Say(ThirdSimpleFixureLongNpc.KillAttackChat[index], 1, 1000);
    ...
    public override void OnKillPlayerSay()
    {
      ...
      int index = this.Game.Random.Next(0, ThirdSimpleFixureLongNpc.KillPlayerChat.Length);
      this.Body.Say(ThirdSimpleFixureLongNpc.KillPlayerChat[index], 1, 0, 2000);
    }
    ...
    {
      int index1 = this.Game.Random.Next(0, ThirdSimpleFixureLongNpc.ShootedChat.Length);
      if (this.isSay == 0 && this.Body.IsLiving)
      {
        this.Body.Say(ThirdSimpleFixureLongNpc.ShootedChat[index1], 1, 900, 0);
        this.isSay = 1;
      }
      if (this.Body.IsLiving)
        return;
      int index2 = this.Game.Random.Next(0, ThirdSimpleFixureLongNpc.DiedChat.Length);
      this.Body.Say(ThirdSimpleFixureLongNpc.DiedChat[index2], 1, 100, 2000);
    }

    public static string GetOneChat()
    {
      int index = ThirdSimpleFixureLongNpc.random.Next(0, ThirdSimpleFixureLongNpc.listChat.Length);
      return ThirdSimpleFixureLongNpc.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdSimpleFixureLongNpc.random.Next(1, 4) : ThirdSimpleFixureLongNpc.random.Next(1, 3)) : ThirdSimpleFixureLongNpc.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdSimpleFixureLongNpc.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdSimpleFixureLongNpc.random.Next(0, 5000);
