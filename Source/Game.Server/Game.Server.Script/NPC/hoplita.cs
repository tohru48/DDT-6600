...
// Type: GameServerScript.AI.NPC.hoplita
  ...
  public class hoplita : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(hoplita.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = hoplita.random.Next(0, hoplita.listChat.Length);
      return hoplita.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? hoplita.random.Next(1, 4) : hoplita.random.Next(1, 3)) : hoplita.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = hoplita.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = hoplita.random.Next(0, 5000);
