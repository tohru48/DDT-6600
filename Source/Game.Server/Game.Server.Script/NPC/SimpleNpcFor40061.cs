...
// Type: GameServerScript.AI.NPC.SimpleNpcFor40061
  ...
  public class SimpleNpcFor40061 : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(SimpleNpcFor40061.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = SimpleNpcFor40061.random.Next(0, SimpleNpcFor40061.listChat.Length);
      return SimpleNpcFor40061.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? SimpleNpcFor40061.random.Next(1, 4) : SimpleNpcFor40061.random.Next(1, 3)) : SimpleNpcFor40061.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = SimpleNpcFor40061.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = SimpleNpcFor40061.random.Next(0, 5000);
