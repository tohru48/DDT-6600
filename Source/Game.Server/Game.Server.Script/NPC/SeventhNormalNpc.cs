...
// Type: GameServerScript.AI.NPC.SeventhNormalNpc
  ...
  public class SeventhNormalNpc : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(SeventhNormalNpc.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = SeventhNormalNpc.random.Next(0, SeventhNormalNpc.listChat.Length);
      return SeventhNormalNpc.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? SeventhNormalNpc.random.Next(1, 4) : SeventhNormalNpc.random.Next(1, 3)) : SeventhNormalNpc.random.Next(0, 2);
      if (length <= 0)
        return;
