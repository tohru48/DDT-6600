...
// Type: GameServerScript.AI.NPC.SimpleNpcAiForAnt
  ...
  public class SimpleNpcAiForAnt : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(SimpleNpcAiForAnt.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = SimpleNpcAiForAnt.random.Next(0, SimpleNpcAiForAnt.listChat.Length);
      return SimpleNpcAiForAnt.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? SimpleNpcAiForAnt.random.Next(1, 4) : SimpleNpcAiForAnt.random.Next(1, 3)) : SimpleNpcAiForAnt.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = SimpleNpcAiForAnt.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = SimpleNpcAiForAnt.random.Next(0, 5000);
