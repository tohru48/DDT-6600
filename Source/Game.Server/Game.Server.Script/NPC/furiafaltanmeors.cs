...
// Type: GameServerScript.AI.NPC.furiafaltanmeors
...
{
  public class furiafaltanmeors : ABrain
  {
    ...
    {
      "Melhor Correr fora !!!!",
      "Vem qui Vem uheuheuhe",
      "Mecheu com os Cara Errado"
    };
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(furiafaltanmeors.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = furiafaltanmeors.random.Next(0, furiafaltanmeors.listChat.Length);
      return furiafaltanmeors.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? furiafaltanmeors.random.Next(1, 4) : furiafaltanmeors.random.Next(1, 3)) : furiafaltanmeors.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = furiafaltanmeors.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = furiafaltanmeors.random.Next(0, 5000);
