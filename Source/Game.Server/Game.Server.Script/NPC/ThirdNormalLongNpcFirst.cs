...
// Type: GameServerScript.AI.NPC.ThirdNormalLongNpcFirst
...
namespace GameServerScript.AI.NPC
{
  public class ThirdNormalLongNpcFirst : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, ThirdNormalLongNpcFirst.KillAttackChat.Length);
      this.Body.Say(ThirdNormalLongNpcFirst.KillAttackChat[index], 0, 1000);
      this.Body.CurrentDamagePlus = 1f;
    ...
    public override void OnKillPlayerSay()
    {
      ...
      int index = this.Game.Random.Next(0, ThirdNormalLongNpcFirst.KillPlayerChat.Length);
      this.Body.Say(ThirdNormalLongNpcFirst.KillPlayerChat[index], 1, 0, 2000);
    }
    ...
    {
      int index1 = this.Game.Random.Next(0, ThirdNormalLongNpcFirst.ShootedChat.Length);
      if (this.isSay == 0 && this.Body.IsLiving)
      {
        this.Body.Say(ThirdNormalLongNpcFirst.ShootedChat[index1], 1, 900, 0);
        this.isSay = 1;
      }
      if (this.Body.IsLiving)
        return;
      int index2 = this.Game.Random.Next(0, ThirdNormalLongNpcFirst.DiedChat.Length);
      this.Body.Say(ThirdNormalLongNpcFirst.DiedChat[index2], 1, 100, 2000);
    }

    public static string GetOneChat()
    {
      int index = ThirdNormalLongNpcFirst.random.Next(0, ThirdNormalLongNpcFirst.listChat.Length);
      return ThirdNormalLongNpcFirst.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdNormalLongNpcFirst.random.Next(1, 4) : ThirdNormalLongNpcFirst.random.Next(1, 3)) : ThirdNormalLongNpcFirst.random.Next(0, 2);
      if (length <= 0)
        return;
