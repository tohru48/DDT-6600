...
// Type: GameServerScript.AI.NPC.ThirdSimpleLongNpcSecond
...
namespace GameServerScript.AI.NPC
{
  public class ThirdSimpleLongNpcSecond : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, ThirdSimpleLongNpcSecond.KillAttackChat.Length);
      this.Body.Say(ThirdSimpleLongNpcSecond.KillAttackChat[index], 1, 1000);
    ...
    {
      this.Body.MoveTo(this.Game.Random.Next(50, 200), this.Body.Y, "walk", 1000, "", 3, new LivingCallBack(this.NextAttack));
    }
    ...
    private void NextAttack()
    {
      ...
      if (this.Body.ShootPoint(1100, randomPlayer.Y, 58, 1000, 10000, 1, 3f, 2550))
    ...
    public override void OnKillPlayerSay()
    {
      ...
      int index = this.Game.Random.Next(0, ThirdSimpleLongNpcSecond.KillPlayerChat.Length);
      this.Body.Say(ThirdSimpleLongNpcSecond.KillPlayerChat[index], 1, 0, 2000);
    }
    ...
    {
      int index1 = this.Game.Random.Next(0, ThirdSimpleLongNpcSecond.ShootedChat.Length);
      if (this.isSay == 0 && this.Body.IsLiving)
      {
        this.Body.Say(ThirdSimpleLongNpcSecond.ShootedChat[index1], 1, 900, 0);
        this.isSay = 1;
      }
      if (this.Body.IsLiving)
        return;
      int index2 = this.Game.Random.Next(0, ThirdSimpleLongNpcSecond.DiedChat.Length);
      this.Body.Say(ThirdSimpleLongNpcSecond.DiedChat[index2], 1, 100, 2000);
    }

    public static string GetOneChat()
    {
      int index = ThirdSimpleLongNpcSecond.random.Next(0, ThirdSimpleLongNpcSecond.listChat.Length);
      return ThirdSimpleLongNpcSecond.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdSimpleLongNpcSecond.random.Next(1, 4) : ThirdSimpleLongNpcSecond.random.Next(1, 3)) : ThirdSimpleLongNpcSecond.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdSimpleLongNpcSecond.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdSimpleLongNpcSecond.random.Next(0, 5000);
