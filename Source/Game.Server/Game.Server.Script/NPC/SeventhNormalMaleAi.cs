...
// Type: GameServerScript.AI.NPC.SeventhNormalMaleAi
  ...
  public class SeventhNormalMaleAi : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(SeventhNormalMaleAi.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = SeventhNormalMaleAi.random.Next(0, SeventhNormalMaleAi.listChat.Length);
      return SeventhNormalMaleAi.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? SeventhNormalMaleAi.random.Next(1, 4) : SeventhNormalMaleAi.random.Next(1, 3)) : SeventhNormalMaleAi.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = SeventhNormalMaleAi.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = SeventhNormalMaleAi.random.Next(0, 5000);
