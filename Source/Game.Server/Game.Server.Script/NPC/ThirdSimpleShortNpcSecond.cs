...
// Type: GameServerScript.AI.NPC.ThirdSimpleShortNpcSecond
  ...
  public class ThirdSimpleShortNpcSecond : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(ThirdSimpleShortNpcSecond.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = ThirdSimpleShortNpcSecond.random.Next(0, ThirdSimpleShortNpcSecond.listChat.Length);
      return ThirdSimpleShortNpcSecond.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdSimpleShortNpcSecond.random.Next(1, 4) : ThirdSimpleShortNpcSecond.random.Next(1, 3)) : ThirdSimpleShortNpcSecond.random.Next(0, 2);
      if (length <= 0)
        return;
