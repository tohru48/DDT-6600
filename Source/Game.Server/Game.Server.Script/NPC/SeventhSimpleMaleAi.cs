...
// Type: GameServerScript.AI.NPC.SeventhSimpleMaleAi
  ...
  public class SeventhSimpleMaleAi : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(SeventhSimpleMaleAi.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = SeventhSimpleMaleAi.random.Next(0, SeventhSimpleMaleAi.listChat.Length);
      return SeventhSimpleMaleAi.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? SeventhSimpleMaleAi.random.Next(1, 4) : SeventhSimpleMaleAi.random.Next(1, 3)) : SeventhSimpleMaleAi.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = SeventhSimpleMaleAi.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = SeventhSimpleMaleAi.random.Next(0, 5000);
