...
// Type: GameServerScript.AI.NPC.FiveTerrorThirdNpc
  ...
  public class FiveTerrorThirdNpc : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(FiveTerrorThirdNpc.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = FiveTerrorThirdNpc.random.Next(0, FiveTerrorThirdNpc.listChat.Length);
      return FiveTerrorThirdNpc.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? FiveTerrorThirdNpc.random.Next(1, 4) : FiveTerrorThirdNpc.random.Next(1, 3)) : FiveTerrorThirdNpc.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = FiveTerrorThirdNpc.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          FiveTerrorThirdNpc.random.Next(0, 5000);
          ++num;
        }
      }
    }
