...
// Type: GameServerScript.AI.NPC.SeventhSimpleSecondBoss
...
namespace GameServerScript.AI.NPC
{
  public class SeventhSimpleSecondBoss : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, SeventhSimpleSecondBoss.KillAttackChat.Length);
      this.Body.Say(SeventhSimpleSecondBoss.KillAttackChat[index], 1, 0);
    ...
    {
      Player randomPlayer = this.Game.FindRandomPlayer();
      if (randomPlayer == null || !this.Body.ShootPoint(randomPlayer.X, randomPlayer.Y, 83, 1000, 10000, 1, 1f, 3300))
        return;
      ...
      if (!this.IsEixt && randomPlayer != null)
      {
        ...
        foreach (Player allLivingPlayer in this.Game.GetAllLivingPlayers())
        {
          ...
          if (allLivingPlayer.X > randomPlayer.X - num && allLivingPlayer.X < randomPlayer.X + num)
            allLivingPlayer.AddEffect((AbstractEffect) new ContinueReduceBlood(2, 200, (Living) allLivingPlayer), 0);
        }
      }
