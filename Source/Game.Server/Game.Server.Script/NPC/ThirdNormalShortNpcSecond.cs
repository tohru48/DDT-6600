...
// Type: GameServerScript.AI.NPC.ThirdNormalShortNpcSecond
  ...
  public class ThirdNormalShortNpcSecond : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(ThirdNormalShortNpcSecond.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = ThirdNormalShortNpcSecond.random.Next(0, ThirdNormalShortNpcSecond.listChat.Length);
      return ThirdNormalShortNpcSecond.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdNormalShortNpcSecond.random.Next(1, 4) : ThirdNormalShortNpcSecond.random.Next(1, 3)) : ThirdNormalShortNpcSecond.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdNormalShortNpcSecond.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdNormalShortNpcSecond.random.Next(0, 5000);
