...
// Type: GameServerScript.AI.NPC.ThirdTerrorShortNpcSecond
  ...
  public class ThirdTerrorShortNpcSecond : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(ThirdTerrorShortNpcSecond.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = ThirdTerrorShortNpcSecond.random.Next(0, ThirdTerrorShortNpcSecond.listChat.Length);
      return ThirdTerrorShortNpcSecond.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdTerrorShortNpcSecond.random.Next(1, 4) : ThirdTerrorShortNpcSecond.random.Next(1, 3)) : ThirdTerrorShortNpcSecond.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdTerrorShortNpcSecond.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdTerrorShortNpcSecond.random.Next(0, 5000);
