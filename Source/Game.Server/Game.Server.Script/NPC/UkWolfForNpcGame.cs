...
// Type: GameServerScript.AI.NPC.UkWolfForNpcGame
...
{
  public class UkWolfForNpcGame : ABrain
  {
    protected Player m_targer;
    private static Random random = new Random();
    private static string[] listChat = new string[5]
    {
      "Tenho fome e sede,<br>quero sua carne e sangue!",
      "Morder, morder, comer, comer, rasgar, rasgar...<br>Irei em frente.",
      "Não subestime um Lobo faminto!",
      "Nossa refeição chegou, todos se satisfaçam...<br>rápido!",
      "Uma feição tão deliciosa assim não pode ser disperdiçada."
    };
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(UkWolfForNpcGame.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = UkWolfForNpcGame.random.Next(0, UkWolfForNpcGame.listChat.Length);
      return UkWolfForNpcGame.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? UkWolfForNpcGame.random.Next(1, 4) : UkWolfForNpcGame.random.Next(1, 3)) : UkWolfForNpcGame.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = UkWolfForNpcGame.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = UkWolfForNpcGame.random.Next(0, 5000);
          livings[index].Say(UkWolfForNpcGame.GetOneChat(), 0, delay);
          ++num;
        }
      }
    }
