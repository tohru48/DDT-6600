...
// Type: GameServerScript.AI.NPC.SimpleNpcFor40056
  ...
  public class SimpleNpcFor40056 : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(SimpleNpcFor40056.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = SimpleNpcFor40056.random.Next(0, SimpleNpcFor40056.listChat.Length);
      return SimpleNpcFor40056.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? SimpleNpcFor40056.random.Next(1, 4) : SimpleNpcFor40056.random.Next(1, 3)) : SimpleNpcFor40056.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = SimpleNpcFor40056.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = SimpleNpcFor40056.random.Next(0, 5000);
