...
// Type: GameServerScript.AI.NPC.UKTerrorFinkoLastGame
...
{
  public class UKTerrorFinkoLastGame : ABrain
  {
    private int m_attackTurn = 0;
    private PhysicalObj m_moive = (PhysicalObj) null;
    private static string[] BeatOneKillChat = new string[2]
    {
      "Isso era conhecido como pesadelo secreto",
      "Jamais alguem sirá vivo!"
    };
    ...
    {
      int index = this.Game.Random.Next(0, UKTerrorFinkoLastGame.BeatOneKillChat.Length);
      this.Body.Say(UKTerrorFinkoLastGame.BeatOneKillChat[index], 1, 0);
      this.Body.Direction = this.Game.FindlivingbyDir(this.Body);
