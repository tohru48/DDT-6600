...
// Type: GameServerScript.AI.NPC.ThirdHardLongNpcFirst
...
namespace GameServerScript.AI.NPC
{
  public class ThirdHardLongNpcFirst : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, ThirdHardLongNpcFirst.KillAttackChat.Length);
      this.Body.Say(ThirdHardLongNpcFirst.KillAttackChat[index], 0, 1000);
      this.Body.CurrentDamagePlus = 1f;
    ...
    {
      this.Body.MoveTo(this.Game.Random.Next(1200, 1550), this.Body.Y, "walk", 1000, "", 3, new LivingCallBack(this.NextAttack));
    }
    ...
    private void NextAttack()
    {
      ...
      if (this.Body.ShootPoint(randomPlayer.X, randomPlayer.Y, 58, 1000, 10000, 1, 2.5f, 2550))
    ...
    public override void OnKillPlayerSay()
    {
      ...
      int index = this.Game.Random.Next(0, ThirdHardLongNpcFirst.KillPlayerChat.Length);
      this.Body.Say(ThirdHardLongNpcFirst.KillPlayerChat[index], 1, 0, 2000);
    }
    ...
    {
      int index1 = this.Game.Random.Next(0, ThirdHardLongNpcFirst.ShootedChat.Length);
      if (this.isSay == 0 && this.Body.IsLiving)
      {
        this.Body.Say(ThirdHardLongNpcFirst.ShootedChat[index1], 1, 900, 0);
        this.isSay = 1;
      }
      if (this.Body.IsLiving)
        return;
      int index2 = this.Game.Random.Next(0, ThirdHardLongNpcFirst.DiedChat.Length);
      this.Body.Say(ThirdHardLongNpcFirst.DiedChat[index2], 1, 100, 2000);
    }

    public static string GetOneChat()
    {
      int index = ThirdHardLongNpcFirst.random.Next(0, ThirdHardLongNpcFirst.listChat.Length);
      return ThirdHardLongNpcFirst.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdHardLongNpcFirst.random.Next(1, 4) : ThirdHardLongNpcFirst.random.Next(1, 3)) : ThirdHardLongNpcFirst.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdHardLongNpcFirst.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdHardLongNpcFirst.random.Next(0, 5000);
