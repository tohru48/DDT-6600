...
// Type: GameServerScript.AI.NPC.FiveNormalFourNpc
  ...
  public class FiveNormalFourNpc : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(FiveNormalFourNpc.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = FiveNormalFourNpc.random.Next(0, FiveNormalFourNpc.listChat.Length);
      return FiveNormalFourNpc.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? FiveNormalFourNpc.random.Next(1, 4) : FiveNormalFourNpc.random.Next(1, 3)) : FiveNormalFourNpc.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        ...
        if (!livings[index].IsSay)
        {
          ...
          FiveNormalFourNpc.random.Next(0, 5000);
          ++num;
        }
      }
    }
