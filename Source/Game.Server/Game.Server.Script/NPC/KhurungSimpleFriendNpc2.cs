...
// Type: GameServerScript.AI.NPC.KhurungSimpleFriendNpc2
...
{
  public class KhurungSimpleFriendNpc2 : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, KhurungSimpleFriendNpc2.AddBooldChat.Length);
      this.Body.Say(KhurungSimpleFriendNpc2.AddBooldChat[index], 1, 0);
      ...
      foreach (Living allLivingPlayer in this.Game.GetAllLivingPlayers())
        allLivingPlayer.AddBlood(7000);
    }

    public override void OnKillPlayerSay()
    {
      base.OnKillPlayerSay();
      int index = this.Game.Random.Next(0, KhurungSimpleFriendNpc2.KillPlayerChat.Length);
      this.Body.Say(KhurungSimpleFriendNpc2.KillPlayerChat[index], 1, 0, 2000);
    }

    public override void OnDiedSay()
    {
      int index = this.Game.Random.Next(0, KhurungSimpleFriendNpc2.DiedChat.Length);
      this.Body.Say(KhurungSimpleFriendNpc2.DiedChat[index], 1, 0, 1500);
    }

    public override void OnShootedSay()
    {
      int index1 = this.Game.Random.Next(0, KhurungSimpleFriendNpc2.ShootedChat.Length);
      if (this.isSay == 0 && this.Body.IsLiving)
      {
        this.Body.Say(KhurungSimpleFriendNpc2.ShootedChat[index1], 1, 900, 0);
        this.isSay = 1;
      }
      if (this.Body.IsLiving)
        return;
      int index2 = this.Game.Random.Next(0, KhurungSimpleFriendNpc2.DiedChat.Length);
      this.Body.Say(KhurungSimpleFriendNpc2.DiedChat[index2], 1, 100, 2000);
    }
  }
}
