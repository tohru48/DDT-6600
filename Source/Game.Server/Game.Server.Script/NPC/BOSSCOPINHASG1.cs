...
// Type: GameServerScript.AI.NPC.BOSSCOPINHASG1
...
{
  public class BOSSCOPINHASG1 : ABrain
  {
    protected Player m_targer;
    private static Random random = new Random();
    private static string[] listChat = new string[6]
    {
      "Eae Otario, vai me matar?",
      "Vamos seu marica, me mata...",
      "Seu bostinha kkkk",
      "Chama seus amigos para jogar também cara!",
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(BOSSCOPINHASG1.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = BOSSCOPINHASG1.random.Next(0, BOSSCOPINHASG1.listChat.Length);
      return BOSSCOPINHASG1.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? BOSSCOPINHASG1.random.Next(1, 4) : BOSSCOPINHASG1.random.Next(1, 3)) : BOSSCOPINHASG1.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = BOSSCOPINHASG1.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = BOSSCOPINHASG1.random.Next(0, 5000);
