...
// Type: GameServerScript.AI.NPC.ThirdTerrorLongNpcFirst
...
namespace GameServerScript.AI.NPC
{
  public class ThirdTerrorLongNpcFirst : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, ThirdTerrorLongNpcFirst.KillAttackChat.Length);
      this.Body.Say(ThirdTerrorLongNpcFirst.KillAttackChat[index], 0, 1000);
      this.Body.CurrentDamagePlus = 1f;
      Player randomPlayer = this.Game.FindRandomPlayer();
      this.Body.MoveTo(this.Game.Random.Next(randomPlayer.X - 70, randomPlayer.X + 70), randomPlayer.Y, "walk", 1000, "", 3);
      this.Body.Direction = this.Game.FindlivingbyDir(this.Body);
    ...
    {
      this.Body.MoveTo(this.Body.X + this.Game.Random.Next(-50, 50), this.Body.Y, "walk", 1000, "", 3, new LivingCallBack(this.NextAttack));
    }
    ...
    private void NextAttack()
    {
      ...
      if (this.Body.ShootPoint(randomPlayer.X, randomPlayer.Y, 58, 1000, 10000, 3, 2.5f, 2550))
    ...
    public override void OnKillPlayerSay()
    {
      ...
      int index = this.Game.Random.Next(0, ThirdTerrorLongNpcFirst.KillPlayerChat.Length);
      this.Body.Say(ThirdTerrorLongNpcFirst.KillPlayerChat[index], 1, 0, 2000);
    }
    ...
    {
      int index1 = this.Game.Random.Next(0, ThirdTerrorLongNpcFirst.ShootedChat.Length);
      if (this.isSay == 0 && this.Body.IsLiving)
      {
        this.Body.Say(ThirdTerrorLongNpcFirst.ShootedChat[index1], 1, 900, 0);
        this.isSay = 1;
      }
      if (this.Body.IsLiving)
        return;
      int index2 = this.Game.Random.Next(0, ThirdTerrorLongNpcFirst.DiedChat.Length);
      this.Body.Say(ThirdTerrorLongNpcFirst.DiedChat[index2], 1, 100, 2000);
    }

    public static string GetOneChat()
    {
      int index = ThirdTerrorLongNpcFirst.random.Next(0, ThirdTerrorLongNpcFirst.listChat.Length);
      return ThirdTerrorLongNpcFirst.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdTerrorLongNpcFirst.random.Next(1, 4) : ThirdTerrorLongNpcFirst.random.Next(1, 3)) : ThirdTerrorLongNpcFirst.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdTerrorLongNpcFirst.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdTerrorLongNpcFirst.random.Next(0, 5000);
