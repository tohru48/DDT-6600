...
// Type: GameServerScript.AI.NPC.GuardasAlmirante
...
{
  public class GuardasAlmirante : ABrain
  {
    protected Player m_targer;
    private static Random random = new Random();
    private static string[] listChat = new string[7]
    {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(GuardasAlmirante.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = GuardasAlmirante.random.Next(0, GuardasAlmirante.listChat.Length);
      return GuardasAlmirante.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? GuardasAlmirante.random.Next(1, 4) : GuardasAlmirante.random.Next(1, 3)) : GuardasAlmirante.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = GuardasAlmirante.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = GuardasAlmirante.random.Next(0, 5000);
