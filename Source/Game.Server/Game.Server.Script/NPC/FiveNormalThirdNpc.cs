...
// Type: GameServerScript.AI.NPC.FiveNormalThirdNpc
  ...
  public class FiveNormalThirdNpc : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(FiveNormalThirdNpc.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = FiveNormalThirdNpc.random.Next(0, FiveNormalThirdNpc.listChat.Length);
      return FiveNormalThirdNpc.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? FiveNormalThirdNpc.random.Next(1, 4) : FiveNormalThirdNpc.random.Next(1, 3)) : FiveNormalThirdNpc.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        ...
        if (!livings[index].IsSay)
        {
          ...
          FiveNormalThirdNpc.random.Next(0, 5000);
          ++num;
        }
      }
    }
