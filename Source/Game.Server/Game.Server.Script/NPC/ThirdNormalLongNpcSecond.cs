...
// Type: GameServerScript.AI.NPC.ThirdNormalLongNpcSecond
...
namespace GameServerScript.AI.NPC
{
  public class ThirdNormalLongNpcSecond : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, ThirdNormalLongNpcSecond.KillAttackChat.Length);
      this.Body.Say(ThirdNormalLongNpcSecond.KillAttackChat[index], 1, 1000);
    ...
    public override void OnKillPlayerSay()
    {
      ...
      int index = this.Game.Random.Next(0, ThirdNormalLongNpcSecond.KillPlayerChat.Length);
      this.Body.Say(ThirdNormalLongNpcSecond.KillPlayerChat[index], 1, 0, 2000);
    }
    ...
    {
      int index1 = this.Game.Random.Next(0, ThirdNormalLongNpcSecond.ShootedChat.Length);
      if (this.isSay == 0 && this.Body.IsLiving)
      {
        this.Body.Say(ThirdNormalLongNpcSecond.ShootedChat[index1], 1, 900, 0);
        this.isSay = 1;
      }
      if (this.Body.IsLiving)
        return;
      int index2 = this.Game.Random.Next(0, ThirdNormalLongNpcSecond.DiedChat.Length);
      this.Body.Say(ThirdNormalLongNpcSecond.DiedChat[index2], 1, 100, 2000);
    }

    public static string GetOneChat()
    {
      int index = ThirdNormalLongNpcSecond.random.Next(0, ThirdNormalLongNpcSecond.listChat.Length);
      return ThirdNormalLongNpcSecond.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdNormalLongNpcSecond.random.Next(1, 4) : ThirdNormalLongNpcSecond.random.Next(1, 3)) : ThirdNormalLongNpcSecond.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdNormalLongNpcSecond.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdNormalLongNpcSecond.random.Next(0, 5000);
