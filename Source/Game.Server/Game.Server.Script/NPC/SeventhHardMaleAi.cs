...
// Type: GameServerScript.AI.NPC.SeventhHardMaleAi
  ...
  public class SeventhHardMaleAi : ABrain
  {
    ...
    public override void OnBeginNewTurn()
    {
      ...
      if (!this.m_body.IsSay)
        return;
      this.m_body.Say(SeventhHardMaleAi.GetOneChat(), 0, this.Game.Random.Next(0, 5000));
    }
    ...
    {
      int index = SeventhHardMaleAi.random.Next(0, SeventhHardMaleAi.listChat.Length);
      return SeventhHardMaleAi.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? SeventhHardMaleAi.random.Next(1, 4) : SeventhHardMaleAi.random.Next(1, 3)) : SeventhHardMaleAi.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = SeventhHardMaleAi.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = SeventhHardMaleAi.random.Next(0, 5000);
