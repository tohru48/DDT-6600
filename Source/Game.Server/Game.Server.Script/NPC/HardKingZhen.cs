...
// Type: GameServerScript.AI.NPC.HardKingZhen
...
namespace GameServerScript.AI.NPC
{
  public class HardKingZhen : ABrain
  {
    ...
    public void HalfAttack()
    {
      ...
      int index = this.Game.Random.Next(0, HardKingZhen.SealChat.Length);
      this.Body.Say(HardKingZhen.AllAttackChat[index], 1, 500);
    ...
    public void Summon()
    {
      int index = this.Game.Random.Next(0, HardKingZhen.CallChat.Length);
      this.Body.Say(HardKingZhen.CallChat[index], 1, 0);
    ...
    {
      int index = this.Game.Random.Next(0, HardKingZhen.SealChat.Length);
      this.Body.Say(HardKingZhen.SealChat[index], 1, 0);
    ...
    public void Angger()
    {
      int index = this.Game.Random.Next(0, HardKingZhen.AngryChat.Length);
      this.Body.Say(HardKingZhen.AngryChat[index], 1, 0);
      this.Body.State = 1;
      this.Dander += 100;
    ...
    public void KillAttack(int fx, int tx)
    {
      ...
      int index = this.Game.Random.Next(0, HardKingZhen.KillAttackChat.Length);
      this.Body.Say(HardKingZhen.KillAttackChat[index], 1, 500);
      this.Body.PlayMovie("beatB", 2500, 0);
