...
// Type: GameServerScript.AI.NPC.ListPmFireworks
...
namespace GameServerScript.AI.NPC
{
  public class ListPmFireworks : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, ListPmFireworks.KillAttackChat.Length);
      if (this.m_turn == 0)
      {
        this.Body.CurrentDamagePlus = 10f;
        this.Body.Say(ListPmFireworks.KillAttackChat[index], 1, 13000);
      ...
      {
        this.Body.CurrentDamagePlus = 10f;
        this.Body.Say(ListPmFireworks.KillAttackChat[index], 1, 0);
        this.Body.PlayMovie("beat1", 2000, 0);
        this.Body.RangeAttacking(fx, tx, "cry", 4000, (List<Player>) null);
      }
    }

    private void ToA() => this.Body.PlayMovie("toA", 1000, 3000);

    private void BeatA()
    {
      this.Body.PlayMovie("beatA", 1000, 9000);
      this.Body.RangeAttacking(this.Body.X + 9000, this.Body.X - 9000, "cry", 3000, (List<Player>) null);
    }
