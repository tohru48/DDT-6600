...
// Type: GameServerScript.AI.NPC.ThirdTerrorKingFour
...
namespace GameServerScript.AI.NPC
{
  public class ThirdTerrorKingFour : ABrain
  {
    ...
    public void HalfAttack()
    {
      ...
      int index = this.Game.Random.Next(0, ThirdTerrorKingFour.SealChat.Length);
      this.Body.Say(ThirdTerrorKingFour.AllAttackChat[index], 1, 500);
    ...
    private void PersonalAttackDame()
    {
      ...
      int index = this.Game.Random.Next(0, ThirdTerrorKingFour.ShootChat.Length);
      this.Body.Say(ThirdTerrorKingFour.ShootChat[index], 1, 500);
      int num;
      if (this.Body.X > randomPlayer.X)
      {
        ...
        if (!this.Body.ShootPoint(randomPlayer.X, randomPlayer.Y, 53, 1000, 10000, 3, 2.3f, 6600))
          return;
        this.Body.PlayMovie("aim", 5000, 0);
        this.Body.PlayMovie("beatA", 6500, 0);
      }
      else
      {
        if (randomPlayer.X > this.Body.Y)
        ...
        if (this.Body.ShootPoint(randomPlayer.X, randomPlayer.Y, 53, 1000, 10000, 3, 1f, 6600))
        {
          this.Body.PlayMovie("aim", 5000, 0);
          this.Body.PlayMovie("beatA", 6500, 0);
        }
    ...
    {
      int index = this.Game.Random.Next(0, ThirdTerrorKingFour.CallChat.Length);
      this.Body.Say(ThirdTerrorKingFour.CallChat[index], 1, 500);
      if (this.IsEixt == 1)
      {
        this.Body.ChangeDirection(1, 50);
    ...
    public void Summon()
    {
      ...
      else
      {
        ...
        int index = this.Game.Random.Next(0, ThirdTerrorKingFour.KillPlayerChat.Length);
        this.Body.Say(ThirdTerrorKingFour.KillPlayerChat[index], 1, 500);
    ...
    public void MovePlayer()
    {
      ...
      else
      {
        ...
        this.Game.FindRandomPlayer();
        int index = this.Game.Random.Next(0, ThirdTerrorKingFour.SealChat.Length);
        this.Body.Say(ThirdTerrorKingFour.SealChat[index], 1, 500);
    ...
    public void KillAttack(int fx, int mx)
    {
      ...
      int index = this.Game.Random.Next(0, ThirdTerrorKingFour.KillAttackChat.Length);
      this.Body.Say(ThirdTerrorKingFour.KillAttackChat[index], 1, 500);
      this.Body.PlayMovie("beatC", 2500, 0);
