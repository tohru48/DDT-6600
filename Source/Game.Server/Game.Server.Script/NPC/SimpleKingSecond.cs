...
// Type: GameServerScript.AI.NPC.SimpleKingSecond
...
namespace GameServerScript.AI.NPC
{
  public class SimpleKingSecond : ABrain
  {
    ...
    {
      this.Body.MoveTo(this.Game.Random.Next(660, 840), this.Body.Y, "walk", 0, "", ((SimpleBoss) this.Body).NpcInfo.speed, new LivingCallBack(this.NextAttack));
    }
    ...
    private void AllAttack()
    {
      ...
      if (this.m_turn == 0)
      {
        int index = this.Game.Random.Next(0, SimpleKingSecond.AllAttackChat.Length);
        this.Body.Say(SimpleKingSecond.AllAttackChat[index], 1, 13000);
      ...
      {
        int index = this.Game.Random.Next(0, SimpleKingSecond.AllAttackChat.Length);
        this.Body.Say(SimpleKingSecond.AllAttackChat[index], 1, 0);
    ...
    {
      int index = this.Game.Random.Next(0, SimpleKingSecond.KillAttackChat.Length);
      if (this.m_turn == 0)
      {
        this.Body.CurrentDamagePlus = 10f;
        this.Body.Say(SimpleKingSecond.KillAttackChat[index], 1, 13000);
        this.Body.PlayMovie("beat1", 15000, 0);
      ...
      {
        this.Body.CurrentDamagePlus = 10f;
        this.Body.Say(SimpleKingSecond.KillAttackChat[index], 1, 0);
    ...
    private void ProtectingWall()
    {
      ...
      int index = this.Game.Random.Next(0, SimpleKingSecond.WallChat.Length);
      this.Body.Say(SimpleKingSecond.WallChat[index], 1, 0);
    }

    public void CreateChild()
    {
      this.Body.PlayMovie("renew", 100, 2000);
      ((SimpleBoss) this.Body).CreateChild(2, 520, 530, 400, 6, 1);
    }
    ...
    private void NextAttack()
    {
      ...
      for (int index1 = 0; index1 < num; ++index1)
      {
        ...
        int index2 = this.Game.Random.Next(0, SimpleKingSecond.ShootChat.Length);
        this.Body.Say(SimpleKingSecond.ShootChat[index2], 1, 0);
