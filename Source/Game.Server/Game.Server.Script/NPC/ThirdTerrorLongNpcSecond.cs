...
// Type: GameServerScript.AI.NPC.ThirdTerrorLongNpcSecond
...
namespace GameServerScript.AI.NPC
{
  public class ThirdTerrorLongNpcSecond : ABrain
  {
    ...
    {
      int index = this.Game.Random.Next(0, ThirdTerrorLongNpcSecond.KillAttackChat.Length);
      this.Body.Say(ThirdTerrorLongNpcSecond.KillAttackChat[index], 1, 1000);
    ...
    public override void OnKillPlayerSay()
    {
      ...
      int index = this.Game.Random.Next(0, ThirdTerrorLongNpcSecond.KillPlayerChat.Length);
      this.Body.Say(ThirdTerrorLongNpcSecond.KillPlayerChat[index], 1, 0, 2000);
    }
    ...
    {
      int index1 = this.Game.Random.Next(0, ThirdTerrorLongNpcSecond.ShootedChat.Length);
      if (this.isSay == 0 && this.Body.IsLiving)
      {
        this.Body.Say(ThirdTerrorLongNpcSecond.ShootedChat[index1], 1, 900, 0);
        this.isSay = 1;
      }
      if (this.Body.IsLiving)
        return;
      int index2 = this.Game.Random.Next(0, ThirdTerrorLongNpcSecond.DiedChat.Length);
      this.Body.Say(ThirdTerrorLongNpcSecond.DiedChat[index2], 1, 100, 2000);
    }

    public static string GetOneChat()
    {
      int index = ThirdTerrorLongNpcSecond.random.Next(0, ThirdTerrorLongNpcSecond.listChat.Length);
      return ThirdTerrorLongNpcSecond.listChat[index];
    }
    ...
    public static void LivingSay(List<Living> livings)
    {
      ...
      foreach (Living living in livings)
        living.IsSay = false;
      int length = count > 5 ? (count <= 5 || count > 10 ? ThirdTerrorLongNpcSecond.random.Next(1, 4) : ThirdTerrorLongNpcSecond.random.Next(1, 3)) : ThirdTerrorLongNpcSecond.random.Next(0, 2);
      if (length <= 0)
        return;
      ...
      while (num < length)
      {
        int index = ThirdTerrorLongNpcSecond.random.Next(0, count);
        if (!livings[index].IsSay)
        {
          livings[index].IsSay = true;
          int delay = ThirdTerrorLongNpcSecond.random.Next(0, 5000);
