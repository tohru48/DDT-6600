...
// Type: GameServerScript.AI.NPC.ThirteenTerrorShadowBoss
...
namespace GameServerScript.AI.NPC
{
  public class ThirteenTerrorShadowBoss : ABrain
  {
    ...
    public override void OnStartAttacking()
    {
      ...
      {
        if (allFightPlayer.IsLiving && allFightPlayer.X > 1830)
        {
    ...
    {
      this.m_moive = (PhysicalObj) ((PVEGame) this.Game).Createlayer(1100, this.Body.Y, "moive", "asset.game.ten.tedabiaoji", "out", 1, 0);
    }
    ...
    {
      int index = this.Game.Random.Next(0, ThirteenTerrorShadowBoss.AngryChat.Length);
      this.Body.Say(ThirteenTerrorShadowBoss.AngryChat[index], 1, 0);
      this.Body.PlayMovie("beatD", 3000, 0);
      this.Body.CurrentDamagePlus = 10f;
      this.Body.RangeAttacking(550, this.Body.X, "cry", 4000, (List<Player>) null);
      this.Body.CallFuction(new LivingCallBack(this.Out), 5000);
    }
    ...
    {
      this.Body.CurrentDamagePlus = 10f;
      int index = this.Game.Random.Next(0, ThirteenTerrorShadowBoss.KillAttackChat.Length);
      this.Body.Say(ThirteenTerrorShadowBoss.KillAttackChat[index], 1, 500);
