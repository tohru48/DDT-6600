	...
	public class GameMgr
	{
		...
		private static void smethod_0()
		{
			...
			while (bool_0)
			{
				...
				{
					ilog_0.Error("Game Mgr Thread Error:", exception);
				}
				long tickCount2 = TickHelper.GetTickCount();
				num += THREAD_INTERVAL - (tickCount2 - tickCount);
				if (tickCount2 - tickCount > THREAD_INTERVAL * 2)
				{
					ilog_0.WarnFormat("Game Mgr spent too much times: {0} ms, count:{1}", tickCount2 - tickCount, num2);
				}
				if (num > 0)
				{
		...
		private static void smethod_1(object object_0)
		{
			...
			foreach (BaseGame item in arrayList)
			{
				...
				{
					ilog_0.Error("game dispose error:", exception);
				}
			}
		}
		...
		private static int smethod_2(long long_1)
		{
			...
			if (allGame != null)
			{
				foreach (BaseGame item in allGame)
				{
					...
					{
						ilog_0.Error("Game  updated error:", exception);
					}
				}
		...
		public static BaseGame StartPVPGame(int roomId, List<IGamePlayer> red, List<IGamePlayer> blue, int mapIndex, eRoomType roomType, eGameType gameType, int timeType)
		{
			try
			{
				int mapIndex2 = MapMgr.GetMapIndex(mapIndex, (byte)roomType, int_0);
			...
			{
				ilog_0.Error("Create game error:", exception);
				return null;
			}
		}
		...
		public static BaseGame StartPVEGame(int roomId, List<IGamePlayer> players, int copyId, eRoomType roomType, eGameType gameType, int timeType, eHardLevel hardLevel, int levelLimits, int currentFloor)
		{
			try
			{
				PveInfo pveInfo = ((copyId == 0 || copyId == 100000) ? PveInfoMgr.GetPveInfoByType(roomType, levelLimits) : PveInfoMgr.GetPveInfoById(copyId));
				if (pveInfo != null)
				{
