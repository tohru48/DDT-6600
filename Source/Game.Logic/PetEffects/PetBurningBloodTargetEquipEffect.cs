...
namespace Game.Logic.PetEffects
{
  public class PetBurningBloodTargetEquipEffect : BasePetEffect
  {
    ...
    {
      if (!(living.PetEffectList.GetOfType(ePetEffectType.PetBurningBloodTargetEquipEffect) is PetBurningBloodTargetEquipEffect ofType))
    ...
    private void method_0(Living living_0, Living living_1, int int_5, int int_6)
    {
      if (!this.IsTrigger || living_0 == living_1 || !(living_1 is Player))
        return;
      living_1.AddPetEffect((AbstractPetEffect) new PetReduceBloodAllBattleEquip(this.int_1, this.int_4, this.Info.ID.ToString()), 0);
      this.IsTrigger = false;
      living_1.Game.udqMkhsej5(living_1, this.Info, true);
    }
