...
namespace Game.Logic.PetEffects
{
  public class PetEnemyAttackBurningBloodEquipEffect : BasePetEffect
  {
    ...
    {
      if (!(living.PetEffectList.GetOfType(ePetEffectType.PetEnemyAttackBurningBloodEquipEffect) is PetEnemyAttackBurningBloodEquipEffect ofType))
    ...
    private void method_0(Living living_0)
    {
      if (this.rand.Next(10000) >= this.int_2 || living_0.PetEffects.CurrentUseSkill != this.int_4)
        return;
      new PetBurningBloodShootingEquip(this.int_1, this.int_4, this.Info.ID.ToString()).Start(living_0);
